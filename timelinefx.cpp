...
void tfx__update_emitter_max_life(tfx_effect_descriptor effect) {
	...
	for (tfx_graph_t &graph : effect->library->graphs[effect->graph_list_index].graphs) {
		if (tfx__is_overtime_graph(&graph) || tfx__is_factor_graph(&graph) || tfx__is_overlength_graph(&graph)) {
			graph.lookup.life = effect->max_life;
		}
	}
}
...
tfx_particle_manager_mode tfx__get_required_particle_manager_mode(tfx_effect_descriptor effect) {
	if (effect->type == tfxEffectType) {
		...
		for (tfx_effect_descriptor sub_effect : effect->children) {
...
float tfx__get_emission_direciton_2d(tfx_particle_manager pm, tfx_library library, tfx_random_t *random, tfx_emitter_state_t &emitter, tfx_vec2_t local_position, tfx_vec2_t world_position) {
	...
	float emission_angle = tfx__lookup_precise(&library->graphs[emitter.graph_list_index].graphs[tfxEmitter_property_emission_pitch_index], emitter.frame);
	float emission_angle_variation = tfx__lookup_precise(&library->graphs[emitter.graph_list_index].graphs[tfxEmitter_property_emission_range_index], emitter.frame);
	//----Emission
...
void tfx__reset_emitter_graphs(tfx_effect_descriptor effect, bool add_node, bool compile) {
	...
	if (compile) {
		tfx__compile_library_overtime_graphs(library, graph_list_index);
	}
}
...
void tfx__compile_library_overtime_graphs(tfx_library library, tfxU32 index, bool including_color_ramps) {
	...
	if (including_color_ramps) {
		tfx__compile_library_color_graphs(library, index);
	}
}
...
void tfx__compile_color_overtime(tfx_graph_t *graph, float gamma) {
	if (graph->nodes.size() > 1) {
		...
		for (tfxU32 f = 0; f != graph->lookup.last_frame + 1; ++f) {
			float age = ((float)f / tfxLOOKUP_TABLE_ARRAY_SIZE) * graph->lookup.life;
			graph->lookup.values[f] = tfx__gamma_correct(tfx__get_linear_graph_value_by_percent_of_life(graph, age, graph->lookup.life), gamma);
		}
...
void tfx__compile_color_ramp(tfx_graph_list_t *graph_list, tfx_color_ramp_t *color_ramp, float gamma) {
	float r, g, b, a;
	...
	if (color_ramp->flags & tfxColorRampFlags_use_sinusoidal_ramp_generation) {
		for (tfxU32 f = 0; f != tfxCOLOR_RAMP_WIDTH; ++f) {
			...
			a = tfx__gamma_correct(tfx__get_linear_graph_value_by_percent_of_life(blendfactor, age, blendfactor->lookup.life), gamma);
		...
		for (tfxU32 f = 0; f != tfxCOLOR_RAMP_WIDTH; ++f) {
			float age = ((float)f / tfxCOLOR_RAMP_WIDTH) * red->lookup.life;
			r = tfx__gamma_correct(tfx__get_linear_graph_value_by_percent_of_life(red, age, red->lookup.life), gamma);
			g = tfx__gamma_correct(tfx__get_linear_graph_value_by_percent_of_life(green, age, green->lookup.life), gamma);
			b = tfx__gamma_correct(tfx__get_linear_graph_value_by_percent_of_life(blue, age, blue->lookup.life), gamma);
			a = tfx__gamma_correct(tfx__get_linear_graph_value_by_percent_of_life(blendfactor, age, blendfactor->lookup.life), gamma);
			color_ramp->colors[f].r = tfxU32(r * 255.f);
			color_ramp->colors[f].g = tfxU32(g * 255.f);
			color_ramp->colors[f].b = tfxU32(b * 255.f);
...
void tfx__push_translation_points(tfx_effect_descriptor effect, tfx_vector_t<tfx_vec3_t> *points, float frame) {
	TFX_ASSERT(effect->transform_index < effect->library->graphs.size());        //Must be a valid index into the library graphs
	tfx_graph_list_t &graph_list = effect->library->graphs[effect->transform_index];
	tfx_vec3_t point(tfx__lookup_precise(&graph_list.graphs[tfxTransform_translate_x], frame),
		tfx__lookup_precise(&graph_list.graphs[tfxTransform_translate_y], frame),
		tfx__lookup_precise(&graph_list.graphs[tfxTransform_translate_z], frame));
...
tfxEffectID tfx__add_effect_to_particle_manager(tfx_particle_manager pm, tfx_effect_descriptor effect, int buffer, int hierarchy_depth, bool is_sub_emitter, tfxU32 root_effect_index, float add_delayed_spawning) {
	...
	new_effect.effect_flags |= (effect->shared_flags & tfxSharedEmitterPropertyFlags_effect_is_3d);
	...
	for (tfx_effect_descriptor child : effect->children) {
		if (child->shared_flags & tfxSharedEmitterPropertyFlags_enabled && !(child->shared_flags & tfxSharedEmitterPropertyFlags_hidden)) {
			...
			if (child->type == tfxEmitterType) {
				...
				state_flags |= child->shared_flags & tfxSharedEmitterPropertyFlags_single && !(pm->flags & tfxParticleManagerFlags_disable_spawning) ? tfxEmitterStateFlags_is_single : 0;
				...
				if (emitter.particles_index == tfxINVALID) {
					if (!is_sub_emitter) {
						emitter.particles_index = tfx__grab_particle_lists(pm, child->path_hash, (effect->shared_flags & tfxSharedEmitterPropertyFlags_effect_is_3d), 100, child->control_profile);
				...
				if (emitter.shared_flags & tfxSharedEmitterPropertyFlags_spawn_location_source) {
					...
					emitter.spawn_locations_index = tfx__grab_particle_location_lists(pm, child->path_hash, (effect->shared_flags & tfxSharedEmitterPropertyFlags_effect_is_3d), 100);
			...
			/*if (pm->flags & tfxParticleManagerFlags_use_compute_shader && tfx_GetEffectInfo(child)->children.empty()) {
...
void tfx__update_emitter_control_profile(tfx_effect_descriptor emitter) {
	...
	if (emitter->property_flags & tfxEmitterPropertyFlags_use_simple_motion_randomness && tfx__get_graph_max_value(&emitter->library->graphs[emitter->graph_list_index].graphs[tfxEmitter_overtime_motion_randomness_index]) > 0.f) {
		emitter->control_profile |= tfxEmitterControlProfile_motion_randomness;
	}
	if (props->emission_direction == tfxOrbital && shared_properties->emission_type != tfxPoint) {
...
int tfx__add_compute_controller(tfx_particle_manager pm) {
...
void tfx__reset_particle_ptr(tfx_particle_manager pm, void *ptr) {
	pm->new_compute_particle_ptr = ptr;
	pm->new_compute_particle_index = 0;
}

void tfx__reset_controller_ptr(tfx_particle_manager pm, void *ptr) {
	pm->compute_controller_ptr = ptr;
}

void tfx__update_compute(tfx_particle_manager pm, void *sampled_particles, unsigned int sample_size) {
...
tfx_compute_particle_t *tfx__grab_compute_particle(tfx_particle_manager pm, unsigned int layer) {
...
void tfx__free_particle_list(tfx_particle_manager pm, tfxU32 index) {
...
void tfx__free_spawn_location_list(tfx_particle_manager pm, tfxU32 index) {
...
void tfx__order_effect_sprites(tfx_effect_instance_data_t *sprites, tfxU32 layer, tfx_particle_manager pm) {
...
void tfx_UpdateParticleManager(tfx_particle_manager pm, float elapsed_time) {
	tfxPROFILE;
	...
	pm->flags &= ~tfxParticleManagerFlags_has_ribbons_to_draw;
	...
	for (int depth = 0; depth != tfxMAXDEPTH; ++depth) {
		...
		for (int i = 0; i != effects_start_size[depth]; ++i) {
			...
			for (int emitter_index : pm->effects[effect_index.index].emitter_indexes[pm->current_ebuff]) {
				...
				spawn_work_entry->sub_effects = &pm->emitters[emitter_index].source_emitter->children;
	...
	for (int depth = 1; depth != tfxMAXDEPTH; ++depth) {
		for (int i = effects_start_size[depth]; i != pm->effects_in_use[depth][pm->current_ebuff].current_size; ++i) {
			...
			for (int i = effect.emitter_start_size; i != effect.emitter_indexes[pm->current_ebuff].current_size; ++i) {
				...
				pm->emitters[emitter_index].particles_index = tfx__grab_particle_lists(pm, pm->emitters[emitter_index].source_emitter->path_hash, pm->flags & tfxParticleManagerFlags_3d_effects, 100, pm->emitters[emitter_index].control_profile);
				TFX_ASSERT(pm->emitters[emitter_index].particles_index != tfxINVALID);
				effect.emitter_indexes[next_buffer].push_back(emitter_index);
			}
		}
...
void *tfx_GetSpriteImagePointer(tfx_particle_manager pm, tfxU32 property_indexes) {
	TFX_DEPRECATED;
...
void tfx_SoftExpireEffect(tfx_particle_manager pm, tfxEffectID effect_index) {
	pm->effects[effect_index].state_flags |= tfxEmitterStateFlags_stop_spawning;
}

void tfx_HardExpireEffect(tfx_particle_manager pm, tfxEffectID effect_index) {
	pm->effects[effect_index].state_flags |= tfxEmitterStateFlags_stop_spawning;
	pm->effects[effect_index].state_flags |= tfxEmitterStateFlags_remove;
}

void *tfx_GetEffectUserData(tfx_particle_manager pm, tfxEffectID effect_index) {
	return pm->effects[effect_index].user_data;
}

void tfx_GetCapturedInstance3dTransform(tfx_particle_manager pm, tfxU32 layer, tfxU32 index, float out_position[3]) {
...
void tfx_GetCapturedInstance2dTransform(tfx_particle_manager pm, tfxU32 layer, tfxU32 index, float out_position[3]) {
...
void tfx__control_particle_position_path_2d(tfx_work_queue_t *queue, void *data) {
	...
	const tfxWideFloat velocity_adjuster = tfxWideSetSingle(tfx__lookup_precise(&library->graphs[emitter.graph_list_index].graphs[tfxEmitter_overtime_velocity_adjuster_index], emitter.frame));
	const tfxWideInt velocity_turbulance_last_frame = tfxWideSetSinglei(work_entry->graphs->graphs[tfxEmitter_overtime_velocity_turbulance_index].lookup.last_frame);
	const tfxWideInt noise_resolution_last_frame = tfxWideSetSinglei(work_entry->graphs->graphs[tfxEmitter_overtime_noise_resolution_index].lookup.last_frame);
	...
	const tfxWideInt stretch_last_frame = tfxWideSetSinglei(work_entry->graphs->graphs[tfxEmitter_overtime_stretch_index].lookup.last_frame);
	...
	for (tfxU32 i = work_entry->start_index; i != work_entry->wide_end_index; i += tfxDataWidth) {
		...
		const tfxWideFloat lookup_velocity = tfxWideLookupSet(work_entry->graphs->graphs[tfxEmitter_overtime_velocity_index].lookup.values, lookup_frame);
		...
		if (emitter.control_profile & tfxEmitterControlProfile_noise) {
			...
			const tfxWideFloat lookup_noise_resolution = tfxWideMul(tfxWideLookupSet(work_entry->graphs->graphs[tfxEmitter_overtime_noise_resolution_index].lookup.values, lookup_frame), noise_resolution);
			lookup_frame.m = tfxWideMini(tfxWideConverti(life), velocity_turbulance_last_frame);
			const tfxWideFloat lookup_velocity_turbulance = tfxWideLookupSet(work_entry->graphs->graphs[tfxEmitter_overtime_velocity_turbulance_index].lookup.values, lookup_frame);
		...
		if (emitter.control_profile & tfxEmitterControlProfile_stretch) {
			...
			const tfxWideFloat lookup_stretch = tfxWideLookupSet(work_entry->graphs->graphs[tfxEmitter_overtime_stretch_index].lookup.values, lookup_frame);
...
const tfxWideFloat lookup_weight = tfxWideLookupSet(work_entry->graphs->graphs[tfxEmitter_overtime_weight_index].lookup.values, lookup_frame_weight);    \
...
const tfxWideFloat influence = tfxWideMul(tfxWideMul(motion_randomness_base, global_noise), tfxWideLookupSet(work_entry->graphs->graphs[tfxEmitter_overtime_motion_randomness_index].lookup.values, lookup_motion_randomness));    \
...
void tfx__control_particle_position_basic_3d(tfx_work_queue_t *queue, void *data) {
	...
	const tfxWideInt velocity_last_frame = tfxWideSetSinglei(work_entry->graphs->graphs[tfxEmitter_overtime_velocity_index].lookup.last_frame);
	const tfxWideFloat velocity_adjuster = tfxWideSetSingle(tfx__lookup_precise(&library->graphs[emitter.graph_list_index].graphs[tfxEmitter_overtime_velocity_adjuster_index], emitter.frame));
	const tfxWideInt weight_last_frame = tfxWideSetSinglei(work_entry->graphs->graphs[tfxEmitter_overtime_weight_index].lookup.last_frame);
...
void tfx__control_particle_motion_randomness_3d(tfx_work_queue_t *queue, void *data) {
	...
	const tfxWideFloat global_noise = tfxWideSetSingle(work_entry->global_noise);
	const tfxWideInt weight_last_frame = tfxWideSetSinglei(work_entry->graphs->graphs[tfxEmitter_overtime_weight_index].lookup.last_frame);
	const tfxWideInt motion_randomness_last_frame = tfxWideSetSinglei(work_entry->graphs->graphs[tfxEmitter_overtime_motion_randomness_index].lookup.last_frame);
	const tfxWideFloat motion_randomness_base = tfxWideSetSingle(tfx__lookup_precise(&library->graphs[emitter.graph_list_index].graphs[tfxEmitter_overtime_motion_randomness_index], emitter.frame));
	const tfxWideFloat node_count = tfxWideSetSingle(work_entry->node_count);
...
void tfx__control_particle_position_2d(tfx_work_queue_t *queue, void *data) {
	...
	//Noise
	const tfxWideInt velocity_last_frame = tfxWideSetSinglei(work_entry->graphs->graphs[tfxEmitter_overtime_velocity_index].lookup.last_frame);
	const tfxWideInt spin_last_frame = tfxWideSetSinglei(work_entry->graphs->graphs[tfxEmitter_overtime_roll_spin_index].lookup.last_frame);
	const tfxWideFloat velocity_adjuster = tfxWideSetSingle(tfx__lookup_precise(&library->graphs[emitter.graph_list_index].graphs[tfxEmitter_overtime_velocity_adjuster_index], emitter.frame));
	const tfxWideInt weight_last_frame = tfxWideSetSinglei(work_entry->graphs->graphs[tfxEmitter_overtime_weight_index].lookup.last_frame);
	const tfxWideInt direction_last_frame = tfxWideSetSinglei(work_entry->graphs->graphs[tfxEmitter_overtime_direction_index].lookup.last_frame);
	const tfxWideInt stretch_last_frame = tfxWideSetSinglei(work_entry->graphs->graphs[tfxEmitter_overtime_stretch_index].lookup.last_frame);
	const tfxWideFloat stretch = tfxWideSetSingle(work_entry->global_stretch);
	const tfxWideInt motion_randomness_last_frame = tfxWideSetSinglei(work_entry->graphs->graphs[tfxEmitter_overtime_motion_randomness_index].lookup.last_frame);
	const tfxWideFloat motion_randomness_base = tfxWideSetSingle(tfx__lookup_precise(&library->graphs[emitter.graph_list_index].graphs[tfxEmitter_overtime_motion_randomness_index], emitter.frame));
	...
	if (work_entry->properties->emission_direction == tfxOrbital && !(emitter.shared_flags & tfxSharedEmitterPropertyFlags_relative_position) && work_entry->shared_properties->emission_type != tfxPoint) {
		emitter_x = tfxWideSetSingle(emitter.world_position.x);
		emitter_y = tfxWideSetSingle(emitter.world_position.y);
		orbit_non_relative = true;
	}
	else if (work_entry->properties->emission_direction == tfxOrbital && (emitter.shared_flags & tfxSharedEmitterPropertyFlags_relative_position) && work_entry->shared_properties->emission_type != tfxPoint) {
		emitter_x = tfxWideSetSingle(emitter.handle.x);
	...
	for (tfxU32 i = work_entry->start_index; i != work_entry->wide_end_index; i += tfxDataWidth) {
		...
		if (orbit_non_relative) {
			...
			lookup_direction.m = tfxWideLookupSet(work_entry->graphs->graphs[tfxEmitter_overtime_direction_index].lookup.values, lookup_frame);
		...
		if (emitter.control_profile & tfxEmitterControlProfile_noise) {
			...
			tfxWideArrayi lookup_motion_randomness;
			lookup_motion_randomness.m = tfxWideMini(tfxWideConverti(life), motion_randomness_last_frame);
			const tfxWideFloat influence = tfxWideMul(motion_randomness_base, tfxWideLookupSet(work_entry->graphs->graphs[tfxEmitter_overtime_motion_randomness_index].lookup.values, lookup_motion_randomness));
		...
		const tfxWideFloat lookup_spin = tfxWideMul(tfxWideLookupSet(work_entry->graphs->graphs[tfxEmitter_overtime_roll_spin_index].lookup.values, lookup_frame), base_spin);
		lookup_frame.m = tfxWideMini(tfxWideConverti(life), weight_last_frame);
		const tfxWideFloat lookup_weight = tfxWideLookupSet(work_entry->graphs->graphs[tfxEmitter_overtime_weight_index].lookup.values, lookup_frame);
		lookup_frame.m = tfxWideMini(tfxWideConverti(life), stretch_last_frame);
		const tfxWideFloat lookup_stretch = tfxWideLookupSet(work_entry->graphs->graphs[tfxEmitter_overtime_stretch_index].lookup.values, lookup_frame);
		p_stretch.m = tfxWideMul(lookup_stretch, stretch);
...
void tfx__control_ribbon_attributes(tfx_work_queue_t *queue, void *data) {
	...
	for (tfxU32 ribbon_index : ribbon_emitter.ribbon_indexes[pm.current_ebuff]) {
		...
		ribbon.curved_alpha = tfx__pack16bit_sscaled(
			...
			lookup_overtime_callback(&graph_list.graphs[tfxRibbon_overtime_alpha_sharpness_index], age_lerp), 128.f
		);
...
void tfx__control_particle_spin_3d(tfx_work_queue_t *queue, void *data) {
	...
	const tfxWideInt spin_last_frame = tfxWideSetSinglei(work_entry->graphs->graphs[tfxEmitter_overtime_roll_spin_index].lookup.last_frame);
	const tfxWideInt spin_pitch_last_frame = tfxWideSetSinglei(work_entry->graphs->graphs[tfxEmitter_overtime_roll_spin_index].lookup.last_frame);
	const tfxWideInt spin_yaw_last_frame = tfxWideSetSinglei(work_entry->graphs->graphs[tfxEmitter_overtime_roll_spin_index].lookup.last_frame);
...
void tfx__control_particle_color(tfx_work_queue_t *queue, void *data) {
	...
	for (tfxU32 i = work_entry->start_index; i != work_entry->wide_end_index; i += tfxDataWidth) {
		...
		tfxWideFloat lookup_intensity = tfxWideLookupSet(work_entry->graphs->graphs[tfxEmitter_overtime_intensity_index].lookup.values, ramp_index);
		tfxWideFloat dissolve_lerp = tfxWideLookupSet(work_entry->graphs->graphs[tfxEmitter_overtime_curved_alpha_index].lookup.values, ramp_index);
		tfxWideFloat sharpness = tfxWideLookupSet(work_entry->graphs->graphs[tfxEmitter_overtime_alpha_sharpness_index].lookup.values, ramp_index);
		tfxWideFloat gradient_mapper = tfxWideLookupSet(work_entry->graphs->graphs[tfxEmitter_overtime_gradient_mapper_index].lookup.values, ramp_index);
...
void tfx__control_particle_image_frame(tfx_work_queue_t *queue, void *data) {
	...
	for (tfxU32 i = work_entry->start_index; i != work_entry->wide_end_index; i += tfxDataWidth) {
		...
		image_indexes.m = tfxWideOri(color_ramp_indexes, tfxWideAddi(tfxWideConverti(image_frame.m), image_start_index));
...
tfx_effect_index_t *tfx_GetPMEffectBuffer(tfx_particle_manager pm, tfxU32 depth, int *count) {
...
void tfx__toggle_sprites_with_uid(tfx_particle_manager pm, bool switch_on) {
	if (switch_on) {
		for (tfxEachLayer) {
...
void tfx_ReconfigureParticleManager(tfx_particle_manager pm, tfx_particle_manager_mode mode, tfxU32 req_sort_passes, bool is_3d) {
...
void tfx_SetStagingBuffer(tfx_particle_manager pm, void *staging_buffer, tfxU32 size_in_bytes) {
...
*/
tfx_2d_instance_t *tfx_Get2dInstanceBuffer(tfx_particle_manager pm) {
...
* @param pm                       A pointer to an intialised tfx_particle_manager_t.
*/
tfx_3d_instance_t *tfx_Get3dInstanceBuffer(tfx_particle_manager pm) {
	return tfxCastBufferRef(tfx_3d_instance_t, pm->instance_buffer);
}

int tfx_GetInstanceCount(tfx_particle_manager pm) {
	return pm->instance_buffer.current_size;
}

tfx_ribbon_bucket_t *tfx_Get3dRibbonBuffers(tfx_particle_manager pm, tfxKey bucket_hash) {
...
void tfx__reset_ribbon_buffer_requirements(tfx_particle_manager pm) {
...
void tfx__update_ribbon_buffer_requirements(tfx_particle_manager pm) {
	tfx_ribbon_bucket_t *bucket = pm->ribbon_segment_buckets.next_item();
	while (bucket) {
...
bool tfx__next_ribbon_bucket(tfx_particle_manager pm, tfx_ribbon_dispatch_t *ribbon_dispatch) {
	tfx_ribbon_bucket_t *bucket = pm->ribbon_segment_buckets.next_item();
	if (bucket) {
		tfxU32 ribbon_count = bucket->highest_ribbon_index - bucket->lowest_ribbon_index + 1;
...
void tfx_ResetRibbonDispatchIterator(tfx_particle_manager pm) {
...
size_t tfx_GetSegment3dVertexBufferMaxSizeInBytes(tfx_particle_manager pm, tfxU32 vertex_size) {
	tfx_ribbon_buffer_info_t buffer_info = tfx__generate_ribbon_buffer_info(pm->info.ribbon_tessellation);
	return pm->info.max_ribbon_segments * buffer_info.vertices_per_segment * (vertex_size ? vertex_size : sizeof(tfx_3d_ribbon_vertex_t));
}

size_t tfx_GetSegmentIndexBufferMaxSizeInBytes(tfx_particle_manager pm) {
	tfx_ribbon_buffer_info_t buffer_info = tfx__generate_ribbon_buffer_info(pm->info.ribbon_tessellation);
	return pm->info.max_ribbon_segments * sizeof(tfxU32) * buffer_info.indices_per_segment;
}

size_t tfx_GetRibbonBufferMaxSizeInBytes(tfx_particle_manager pm, tfxU32 max_ribbons) {
	return max_ribbons * sizeof(tfx_ribbon_t);
}

size_t tfx_GetEmitterBufferMaxSizeInBytes(tfx_particle_manager pm) {
	return pm->info.max_effects * sizeof(tfx_gpu_emitter_t);
}

void tfx_SetPMWorkQueueSizes(tfx_particle_manager pm, tfxU32 spawn_work_max, tfxU32 control_work_max, tfxU32 age_work_max) {
...
void tfx_ClearParticleManager(tfx_particle_manager pm, bool free_particle_banks, bool free_sprite_buffers) {
	tfx__complete_all_work(&pm->work_queue);
...
void tfx_FreeParticleManager(tfx_particle_manager pm) {
	tfx__free_all_particle_lists(pm);
	for (auto &list : pm->free_particle_lists.data) {
		list.free();
	}
...
void tfx__free_all_particle_lists(tfx_particle_manager pm) {
...
void tfx__free_all_spawn_location_lists(tfx_particle_manager pm) {
...
void tfx_SoftExpireAll(tfx_particle_manager pm) {
...
void tfx_KeepBoundingBoxesUpdated(tfx_particle_manager pm, bool yesno) {
	if (yesno) {
...
void tfx_UpdatePMBaseValues(tfx_particle_manager pm) {
	pm->flags |= tfxParticleManagerFlags_update_base_values;
}

bool tfx__free_pm_effect_capacity(tfx_particle_manager pm) {
...
tfxU32 tfx__get_ribbon_slot(tfx_particle_manager pm) {
...
tfxU32 tfx__allocate_path_quaternion(tfx_particle_manager pm, tfxU32 amount) {
...
void tfx__free_path_quaternion(tfx_particle_manager pm, tfxU32 index) {
...
void tfx_SetPMCamera(tfx_particle_manager pm, float front[3], float position[3]) {
...
void tfx__reset_particle_manager_flags(tfx_particle_manager pm) {
	pm->flags = 0;
}

tfxU32 tfx__get_particle_sprite_index(tfx_particle_manager pm, tfxParticleID id) {
	return pm->particle_arrays[tfx__particle_bank(id)].sprite_index[tfx__particle_index(id)];
}

void tfx__free_compute_slot(tfx_particle_manager pm, unsigned int slot_id) {
	pm->free_compute_controllers.push_back(slot_id);
}

tfxU32 tfx_ParticleCount(tfx_particle_manager pm) {
	return pm->instance_buffer.current_size;
}

tfxU32 tfx_RibbonCount(tfx_particle_manager pm) {
	return pm->current_ribbon_count;
}

tfxU32 tfx_EffectCount(tfx_particle_manager pm) {
	tfxU32 count = 0;
...
tfxU32 tfx_EmitterCount(tfx_particle_manager pm) {
	tfxU32 count = 0;
	for (int d = 0; d != tfxMAXDEPTH; ++d) {
		count += pm->control_emitter_queue.current_size;
	}
...
tfxU32 tfx__grab_particle_lists(tfx_particle_manager pm, tfxKey emitter_hash, bool is_3d, tfxU32 reserve_amount, tfxEmitterControlProfileFlags flags) {
...
tfxU32 tfx__grab_gpu_emitter(tfx_particle_manager pm) {
...
void tfx__free_gpu_emitter(tfx_particle_manager pm, tfxU32 index) {
	pm->free_gpu_emitters.push_back(index);
}

tfxU32 tfx__grab_ribbon(tfx_particle_manager pm, tfx_ribbon_bucket_t *bucket, tfx_ribbon_emitter_state_t *ribbon_emitter) {
...
void tfx__free_ribbon(tfx_particle_manager pm, tfxKey bucket_hash, tfxU32 ribbon_index) {
...
void tfx__init_ribbon_segment_buffer(tfx_particle_manager pm, tfxKey bucket_id, tfx_ribbon_bucket_info_t *bucket_info, int tessellation) {
	...
	tfx__reset_soa_buffer(&bucket.ribbons_buffer);
...
void tfx__update_ribbon_emitter(tfxU32 ribbon_emitter_index, tfx_work_queue_t *work_queue, void *data) {
	...
	translation.x = tfx__lookup_precise(&library->graphs[ribbon_emitter.transform_index].graphs[tfxTransform_translate_x_index], ribbon_emitter.age);
	translation.y = tfx__lookup_precise(&library->graphs[ribbon_emitter.transform_index].graphs[tfxTransform_translate_y_index], ribbon_emitter.age);
	translation.z = tfx__lookup_precise(&library->graphs[ribbon_emitter.transform_index].graphs[tfxTransform_translate_z_index], ribbon_emitter.age);

	ribbon_emitter.captured_position = ribbon_emitter.world_position;

	ribbon_emitter.frame = ribbon_emitter.age;

	local_rotations.roll = tfx__lookup_precise(&library->graphs[ribbon_emitter.transform_index].graphs[tfxTransform_roll_index], ribbon_emitter.age);
	local_rotations.pitch = tfx__lookup_precise(&library->graphs[ribbon_emitter.transform_index].graphs[tfxTransform_pitch_index], ribbon_emitter.age);
	local_rotations.yaw = tfx__lookup_precise(&library->graphs[ribbon_emitter.transform_index].graphs[tfxTransform_yaw_index], ribbon_emitter.age);
	...
	if (ribbon_emitter.shared_flags & tfxSharedEmitterPropertyFlags_effect_is_3d) {
		...
		tfx__transform_2d(&ribbon_emitter.world_rotations, &local_rotations, &ribbon_work_entry->overal_scale, &ribbon_emitter.world_position, &ribbon_emitter.local_position, &translation, &ribbon_emitter.rotation, &parent_effect);
	}
...
void tfx__update_emitter(tfx_work_queue_t *work_queue, void *data) {
	...
	tfx_particle_manager pm = spawn_work_entry->pm;
...
tfxU32 tfx__new_sprites_needed(tfx_particle_manager pm, tfx_random_t *random, tfxU32 index, tfx_effect_state_t *parent, tfx_shared_properties_t *shared_properties) {
	tfx_emitter_state_t &emitter = pm->emitters[index];
	tfx_library library = emitter.library;
	tfx_AlterRandomSeedU32(random, 25 + emitter.seed_index);
	if (!(emitter.shared_flags & tfxSharedEmitterPropertyFlags_single)) {
		emitter.spawn_quantity = tfx__lookup_precise(&library->graphs[emitter.graph_list_index].graphs[tfxEmitter_base_amount_index], emitter.frame);
		float amount_variation = tfx__lookup_precise(&library->graphs[emitter.graph_list_index].graphs[tfxEmitter_variation_amount_index], emitter.frame);
	...
	emitter.spawn_quantity *= tfx__lookup_precise(&library->graphs[parent->graph_list_index].graphs[tfxEffect_global_amount_index], emitter.frame);
...
tfxU32 tfx__new_ribbons_needed(tfx_particle_manager pm, tfx_random_t *random, tfxU32 index, tfx_effect_state_t *parent, tfx_shared_properties_t *shared_properties) {
	tfx_ribbon_emitter_state_t &ribbon_emitter = pm->ribbon_emitters[index];
	tfx_library library = ribbon_emitter.library;
	...
	if (!(ribbon_emitter.shared_flags & tfxSharedEmitterPropertyFlags_single)) {
		ribbon_emitter.spawn_quantity = tfx__lookup_precise(&library->graphs[ribbon_emitter.graph_list_index].graphs[tfxRibbon_base_amount_index], ribbon_emitter.frame);
		float amount_variation = tfx__lookup_precise(&library->graphs[ribbon_emitter.graph_list_index].graphs[tfxRibbon_variation_amount_index], ribbon_emitter.frame);
	...
	ribbon_emitter.spawn_quantity *= tfx__lookup_precise(&library->graphs[parent->graph_list_index].graphs[tfxEffect_global_amount_index], ribbon_emitter.frame);
...
tfxU32 tfx__spawn_particles(tfx_particle_manager pm, tfx_spawn_work_entry_t *work_entry) {
	tfx_emitter_state_t &emitter = pm->emitters[work_entry->emitter_index];
	...
	if (work_entry->amount_to_spawn > 0) {
		if (emitter.state_flags & tfxEmitterStateFlags_is_in_ordered_effect) {
			...
			tfx__add_work_queue_entry(&pm->work_queue, work_entry, pm->flags & tfxParticleManagerFlags_3d_effects ? tfx__do_spawn_work_3d : tfx__do_spawn_work_2d);
		}
	}
...
void tfx__do_spawn_work_3d(tfx_work_queue_t *queue, void *data) {
	...
	if (work_entry->emission_type == tfxOtherEmitter) {
		if (emitter.shared_flags & tfxSharedEmitterPropertyFlags_single) {
			tfx__spawn_particle_other_emitter_single_3d(&pm->work_queue, work_entry);
		} else {
...
void tfx__spawn_particle_age(tfx_work_queue_t *queue, void *data) {
	...
	const float life = tfx__lookup_precise(&library->graphs[emitter.graph_list_index].graphs[tfxEmitter_base_life_index], emitter.frame) * entry->parent_spawn_controls->life;
	const float life_variation = tfx__lookup_precise(&library->graphs[emitter.graph_list_index].graphs[tfxEmitter_variation_life_index], emitter.frame) * entry->parent_spawn_controls->life;
		...
		if (entry->sub_effects->current_size > 0) {
			...
			for (tfx_effect_descriptor sub : *entry->sub_effects) {
				if (!tfx__free_pm_effect_capacity(&pm))
					break;
				if (sub->shared_flags & tfxSharedEmitterPropertyFlags_enabled && !(sub->shared_flags & tfxSharedEmitterPropertyFlags_hidden)) {
...
void tfx__spawn_particle_size_2d(tfx_work_queue_t *queue, void *data) {
	...
	tfx_vec2_t size;
	if (!(emitter.property_flags & tfxEmitterPropertyFlags_base_uniform_size)) {
		size.x = tfx__lookup_precise(&library->graphs[emitter.graph_list_index].graphs[tfxEmitter_base_width_index], emitter.frame) * entry->parent_spawn_controls->size_x;
		size.y = tfx__lookup_precise(&library->graphs[emitter.graph_list_index].graphs[tfxEmitter_base_height_index], emitter.frame) * entry->parent_spawn_controls->size_y;
	}
	else {
		size.x = tfx__lookup_precise(&library->graphs[emitter.graph_list_index].graphs[tfxEmitter_base_width_index], emitter.frame);
	...
	size_variation.x = tfx__lookup_precise(&library->graphs[emitter.graph_list_index].graphs[tfxEmitter_variation_width_index], emitter.frame) * entry->parent_spawn_controls->size_x;
...
void tfx__spawn_particle_noise(tfx_work_queue_t *queue, void *data) {
	...
	float emitter_noise_offset_variation = tfx__lookup_precise(&library->graphs[emitter.graph_list_index].graphs[tfxEmitter_variation_noise_offset_index], emitter.frame);
	float emitter_noise_offset = tfx__lookup_precise(&library->graphs[emitter.graph_list_index].graphs[tfxEmitter_base_noise_offset_index], emitter.frame);
	float emitter_noise_resolution = tfx__lookup_precise(&library->graphs[emitter.graph_list_index].graphs[tfxEmitter_variation_noise_resolution_index], emitter.frame);
...
void tfx__spawn_particle_spin_2d(tfx_work_queue_t *queue, void *data) {
	...
	const float spin = tfx__lookup_precise(&library->graphs[emitter.graph_list_index].graphs[tfxEmitter_base_roll_spin_index], emitter.frame) * entry->parent_spawn_controls->spin;
	const float spin_variation = tfx__lookup_precise(&library->graphs[emitter.graph_list_index].graphs[tfxEmitter_variation_roll_spin_index], emitter.frame) * entry->parent_spawn_controls->spin;
...
void tfx__spawn_particle_spin_3d(tfx_work_queue_t *queue, void *data) {
	...
	const float roll_spin = tfx__lookup_precise(&library->graphs[emitter.graph_list_index].graphs[tfxEmitter_base_roll_spin_index], emitter.frame) * entry->parent_spawn_controls->spin;
	const float pitch_spin = tfx__lookup_precise(&library->graphs[emitter.graph_list_index].graphs[tfxEmitter_base_pitch_spin_index], emitter.frame) * entry->parent_spawn_controls->pitch_spin;
	const float yaw_spin = tfx__lookup_precise(&library->graphs[emitter.graph_list_index].graphs[tfxEmitter_base_yaw_spin_index], emitter.frame) * entry->parent_spawn_controls->yaw_spin;
	const float spin_variation = tfx__lookup_precise(&library->graphs[emitter.graph_list_index].graphs[tfxEmitter_variation_roll_spin_index], emitter.frame) * entry->parent_spawn_controls->spin;
	const float spin_pitch_variation = tfx__lookup_precise(&library->graphs[emitter.graph_list_index].graphs[tfxEmitter_variation_pitch_spin_index], emitter.frame) * entry->parent_spawn_controls->pitch_spin;
	const float spin_yaw_variation = tfx__lookup_precise(&library->graphs[emitter.graph_list_index].graphs[tfxEmitter_variation_yaw_spin_index], emitter.frame) * entry->parent_spawn_controls->yaw_spin;
...
void tfx__spawn_particle_other_emitter_single_3d(tfx_work_queue_t *queue, void *data) {
	tfxPROFILE;
	tfx_spawn_work_entry_t *entry = static_cast<tfx_spawn_work_entry_t *>(data);
	tfx_random_t random = entry->random;
	float tween = 0.f;
...
void tfx__spawn_particle_ellipse_2d(tfx_work_queue_t *queue, void *data) {
	...
	for (int i = 0; i != entry->amount_to_spawn; ++i) {
		...
		if (emitter.shared_flags & tfxSharedEmitterPropertyFlags_spawn_on_grid && !(emitter.shared_flags & tfxSharedEmitterPropertyFlags_fill_area) && !(emitter.shared_flags & tfxSharedEmitterPropertyFlags_grid_spawn_random)) {
		...
		else if (emitter.shared_flags & tfxSharedEmitterPropertyFlags_spawn_on_grid && !(emitter.shared_flags & tfxSharedEmitterPropertyFlags_fill_area) && emitter.shared_flags & tfxSharedEmitterPropertyFlags_grid_spawn_random) {
...
void tfx__spawn_particle_path_2d(tfx_work_queue_t *queue, void *data) {
	...
	if (properties.emission_direction == tfxPathGradient) {
		emission_direction = tfx__lookup_precise(&library->graphs[emitter.graph_list_index].graphs[tfxEmitter_property_emission_pitch_index], emitter.frame);
		emission_angle_variation = tfx__lookup_precise(&library->graphs[emitter.graph_list_index].graphs[tfxEmitter_property_emission_range_index], emitter.frame);
		range = emission_angle_variation * .5f;
	}
...
void tfx__spawn_particle_path_3d(tfx_work_queue_t *queue, void *data) {
	...
	if (properties.emission_direction == tfxPathGradient) {
		emission_pitch = tfx__lookup_precise(&library->graphs[emitter.graph_list_index].graphs[tfxEmitter_property_emission_pitch_index], emitter.frame);
		emission_yaw = tfx__lookup_precise(&library->graphs[emitter.graph_list_index].graphs[tfxEmitter_property_emission_yaw_index], emitter.frame);
		emission_angle_variation = tfx__lookup_precise(&library->graphs[emitter.graph_list_index].graphs[tfxEmitter_property_emission_range_index], emitter.frame);
		range = emission_angle_variation * .5f;
	}
...
void tfx__spawn_static_ribbons(tfxU32 ribbon_emitter_index, tfx_work_queue_t *queue, void *data) {
	...
	const float life = tfx__lookup_precise(&graph_list.graphs[tfxRibbon_base_life_index], ribbon_emitter.frame) * entry->parent_spawn_controls->life;
	const float life_variation = tfx__lookup_precise(&graph_list.graphs[tfxRibbon_variation_life_index], ribbon_emitter.frame) * entry->parent_spawn_controls->life;
	float base_width = tfx__lookup_precise(&graph_list.graphs[tfxRibbon_base_width_index], ribbon_emitter.frame) * entry->parent_spawn_controls->size_x;
	...
	float arc_size = tfx__lookup_precise(&library->graphs[ribbon_emitter.graph_list_index].graphs[tfxRibbon_property_arc_size_index], ribbon_emitter.frame);
	...
	if (ribbon_emitter.static_segment_start_index != tfxINVALID) {
		...
		const float splatter = tfx__lookup_precise(&library->graphs[ribbon_emitter.graph_list_index].graphs[tfxRibbon_property_splatter_index], ribbon_emitter.frame) * entry->parent_spawn_controls->splatter;
		for (int i = 0; i != entry->amount_to_spawn; ++i) {
...
void tfx__spawn_particle_weight(tfx_work_queue_t *queue, void *data) {
	...
	float weight = tfx__lookup_precise(&library->graphs[emitter.graph_list_index].graphs[tfxEmitter_base_weight_index], emitter.frame) * entry->parent_spawn_controls->weight;
	float weight_variation = tfx__lookup_precise(&library->graphs[emitter.graph_list_index].graphs[tfxEmitter_variation_weight_index], emitter.frame) * entry->parent_spawn_controls->weight;
...
void tfx__spawn_particle_micro_update_2d(tfx_work_queue_t *queue, void *data) {
	...
	const tfxAngleSettingFlags angle_settings = properties.angle_settings;
	const float angle_roll_offset = properties.angle_offsets.roll;
	const tfx_emission_type emission_type = entry->shared_properties->emission_type;
	const tfxU32 layer = entry->shared_properties->layer;
	bool is_recording = (pm.flags & tfxParticleManagerFlags_recording_sprites) > 0 && (pm.flags & tfxParticleManagerFlags_using_uids) > 0;
	tfx_buffer_t &instance_buffer = !is_recording ? pm.instance_buffer : pm.instance_buffer_for_recording[pm.current_sprite_buffer][layer];
	...
	for (int i = 0; i != entry->amount_to_spawn; ++i) {
		...
		if (!line) {
			if (properties.emission_direction != tfxPathGradient) {
				direction = tfx__get_emission_direciton_2d(&pm, library, &random, emitter, tfx_vec2_t(local_position_x, local_position_y), sprite_transform_position) + tfx__get_graph_first_value(&library->graphs[emitter.graph_list_index].graphs[tfxEmitter_overtime_direction_index]);
			}
		}
		else {
			direction = 0;
...
void tfx__spawn_particle_micro_update_3d(tfx_work_queue_t *queue, void *data) {
	...
	if (splatter || entry->emission_type == tfxSpawnOnRibbon) {
		for (int i = 0; i != entry->amount_to_spawn; ++i) {
			...
			if (entry->emission_type == tfxSpawnOnRibbon) {
				...
				emitter.grid_coords.x++;
				float max_age = entry->particle_data->max_age[index];
				factor = lookup_overtime_callback(&library->graphs[ribbon_emitter.graph_list_index].graphs[tfxRibbon_overlength_width_index], ribbon_lerp);
			}
	...
	float emission_pitch = tfx__lookup_precise(&library->graphs[emitter.graph_list_index].graphs[tfxEmitter_property_emission_pitch_index], emitter.frame);
	float emission_yaw = tfx__lookup_precise(&library->graphs[emitter.graph_list_index].graphs[tfxEmitter_property_emission_yaw_index], emitter.frame);
	const float first_velocity_value = tfx__get_graph_first_value(&library->graphs[emitter.graph_list_index].graphs[tfxEmitter_overtime_velocity_index]);
	const float first_weight_value = tfx__get_graph_first_value(&library->graphs[emitter.graph_list_index].graphs[tfxEmitter_overtime_weight_index]);
	...
	const float velocity_adjuster = tfx__lookup_precise(&library->graphs[emitter.graph_list_index].graphs[tfxEmitter_overtime_velocity_adjuster_index], emitter.frame);
	const tfxU32 layer = shared_properties.layer;
	tfx_emission_direction emission_direction = library->emitter_properties[emitter.properties_index].emission_direction;
...
void tfx__update_ribbon_emitter_state(tfx_particle_manager pm, tfx_ribbon_emitter_state_t &ribbon, tfxU32 parent_index, const tfx_parent_spawn_controls_t *parent_spawn_controls, tfx_ribbon_work_entry_t *entry) {
	tfxPROFILE;
...
void tfx__update_emitter_state(tfx_particle_manager pm, tfx_emitter_state_t &emitter, tfxU32 parent_index, const tfx_parent_spawn_controls_t *parent_spawn_controls, tfx_spawn_work_entry_t *entry) {
	tfxPROFILE;
...
void tfx__update_effect_state(tfx_particle_manager pm, tfxU32 index) {
	...
	spawn_controls.life = tfx__lookup_precise(&library->graphs[graph_list_index].graphs[tfxEffect_global_life_index], frame);
	if (!(pm->effects[index].effect_flags & tfxEffectPropertyFlags_global_uniform_size)) {
		spawn_controls.size_x = tfx__lookup_precise(&library->graphs[graph_list_index].graphs[tfxEffect_global_width_index], frame);
		spawn_controls.size_y = tfx__lookup_precise(&library->graphs[graph_list_index].graphs[tfxEffect_global_height_index], frame);
	}
	else {
		spawn_controls.size_x = tfx__lookup_precise(&library->graphs[graph_list_index].graphs[tfxEffect_global_width_index], frame);
		spawn_controls.size_y = spawn_controls.size_x;
	}
	spawn_controls.velocity = tfx__lookup_precise(&library->graphs[graph_list_index].graphs[tfxEffect_global_velocity_index], frame);
	noise = tfx__lookup_precise(&library->graphs[graph_list_index].graphs[tfxEffect_global_noise_index], frame);
	spawn_controls.spin = tfx__lookup_precise(&library->graphs[graph_list_index].graphs[tfxEffect_global_roll_spin_index], frame);
	spawn_controls.pitch_spin = tfx__lookup_precise(&library->graphs[graph_list_index].graphs[tfxEffect_global_pitch_spin_index], frame);
	spawn_controls.yaw_spin = tfx__lookup_precise(&library->graphs[graph_list_index].graphs[tfxEffect_global_yaw_spin_index], frame);
	spawn_controls.intensity = tfx__lookup_precise(&library->graphs[graph_list_index].graphs[tfxEffect_global_intensity_index], frame);
	spawn_controls.splatter = tfx__lookup_precise(&library->graphs[graph_list_index].graphs[tfxEffect_global_splatter_index], frame);
	spawn_controls.weight = tfx__lookup_precise(&library->graphs[graph_list_index].graphs[tfxEffect_global_weight_index], frame);
	if (!(state_flags & tfxEffectStateFlags_override_size_multiplier)) {
		emitter_size.x = tfx__lookup_precise(&library->graphs[graph_list_index].graphs[tfxEffect_global_emitter_width_index], frame);
		emitter_size.y = tfx__lookup_precise(&library->graphs[graph_list_index].graphs[tfxEffect_global_emitter_height_index], frame);
		emitter_size.z = tfx__lookup_precise(&library->graphs[graph_list_index].graphs[tfxEffect_global_emitter_depth_index], frame);
	}
	...
	overal_scale = state_flags & tfxEffectStateFlags_override_overal_scale ? overal_scale : tfx__lookup_precise(&library->graphs[graph_list_index].graphs[tfxEffect_global_overal_scale_index], frame);
	if (pm->effects[index].parent_particle_index == tfxINVALID) {
		if (!(state_flags & tfxEffectStateFlags_override_orientiation)) {
			local_rotations.roll = tfx__lookup_precise(&library->graphs[transform_index].graphs[tfxTransform_roll_index], age);
			local_rotations.pitch = tfx__lookup_precise(&library->graphs[transform_index].graphs[tfxTransform_pitch_index], age);
			local_rotations.yaw = tfx__lookup_precise(&library->graphs[transform_index].graphs[tfxTransform_yaw_index], age);
		}
	}
	else {
	...
	pm->effects[index].stretch = tfx__lookup_precise(&library->graphs[graph_list_index].graphs[tfxEffect_global_stretch_index], frame);
	translation.x = tfx__lookup_precise(&library->graphs[transform_index].graphs[tfxTransform_translate_x_index], age);
	translation.y = tfx__lookup_precise(&library->graphs[transform_index].graphs[tfxTransform_translate_y_index], age);
	translation.z = tfx__lookup_precise(&library->graphs[transform_index].graphs[tfxTransform_translate_z_index], age);
...
void tfx__init_sprite_data_soa_compression_3d(tfx_soa_buffer_t *buffer, tfx_sprite_data_soa_t *soa, tfxU32 reserve_amount) {
	...
	tfx__add_struct_array(buffer, sizeof(tfx_3d_instance_t), offsetof(tfx_sprite_data_soa_t, billboard_instance));
...
void tfx__init_sprite_data_soa_compression_2d(tfx_soa_buffer_t *buffer, tfx_sprite_data_soa_t *soa, tfxU32 reserve_amount) {
	tfx__add_struct_array(buffer, sizeof(tfx_unique_sprite_id_t), offsetof(tfx_sprite_data_soa_t, uid));
	tfx__add_struct_array(buffer, sizeof(float), offsetof(tfx_sprite_data_soa_t, lerp_offset));
	tfx__add_struct_array(buffer, sizeof(tfx_2d_instance_t), offsetof(tfx_sprite_data_soa_t, sprite_instance));
	tfx__finish_soa_buffer_setup(buffer, soa, reserve_amount, 16);
}
...
void tfx__init_ribbon_segment_soa_3d(tfx_soa_buffer_t *buffer, tfx_ribbon_segment_soa_t *soa, tfxU32 reserve_amount) {
...
void tfx__init_ribbon_segment_soa_2d(tfx_soa_buffer_t *buffer, tfx_ribbon_segment_soa_t *soa, tfxU32 reserve_amount) {
	tfx__add_struct_array(buffer, sizeof(float), offsetof(tfx_ribbon_segment_soa_t, x));
	tfx__add_struct_array(buffer, sizeof(float), offsetof(tfx_ribbon_segment_soa_t, y));
	tfx__add_struct_array(buffer, sizeof(float), offsetof(tfx_ribbon_segment_soa_t, width));
	tfx__finish_soa_buffer_setup(buffer, soa, reserve_amount, 16);
...
bool tfx__valid_effect_id(tfx_particle_manager pm, tfxEffectID id) {
...
void tfx__init_common_particle_manager(tfx_particle_manager pm, tfxU32 max_particles, unsigned int effects_limit, tfx_particle_manager_mode mode, bool double_buffered_sprites, bool dynamic_sprite_allocation, bool group_sprites_by_effect, tfxU32 mt_batch_size) {
...
tfx_particle_manager tfx_CreateParticleManager(tfx_particle_manager_info_t info) {
	...
	pm->info = info;
	tfx__init_common_particle_manager(pm, info.max_particles, info.max_effects, info.order_mode, info.double_buffer_sprites, info.dynamic_sprite_allocation, info.group_sprites_by_effect, info.multi_threaded_batch_size);
...
void tfx_SetEffectPosition2d(tfx_particle_manager pm, tfxEffectID effect_index, float x, float y) {
...
void tfx_SetEffectPositionVec2(tfx_particle_manager pm, tfxEffectID effect_index, tfx_vec2_t position) {
	TFX_ASSERT(tfx__valid_effect_id(pm, effect_index));    //Not a valid effect id. Make sure that when you call tfx_AddEffectTemplateToParticleManager you check that it returns true.
	pm->effects[effect_index].local_position = position;
}

void tfx_SetEffectPosition3d(tfx_particle_manager pm, tfxEffectID effect_index, float x, float y, float z) {
	TFX_ASSERT(tfx__valid_effect_id(pm, effect_index));    //Not a valid effect id. Make sure that when you call tfx_AddEffectTemplateToParticleManager you check that it returns true.
	tfx_vec3_t position(x, y, z);
	pm->effects[effect_index].local_position = position;
}

void tfx_SetEffectPositionVec3(tfx_particle_manager pm, tfxEffectID effect_index, tfx_vec3_t position) {
	TFX_ASSERT(tfx__valid_effect_id(pm, effect_index));    //Not a valid effect id. Make sure that when you call tfx_AddEffectTemplateToParticleManager you check that it returns true.
	pm->effects[effect_index].local_position = position;
}
...
void tfx_MoveEffectVec3(tfx_particle_manager pm, tfxEffectID effect_index, tfx_vec3_t amount) {
	TFX_ASSERT(tfx__valid_effect_id(pm, effect_index));    //Not a valid effect id. Make sure that when you call tfx_AddEffectTemplateToParticleManager you check that it returns true.
	pm->effects[effect_index].local_position += amount;
}

void tfx_MoveEffect3d(tfx_particle_manager pm, tfxEffectID effect_index, float x, float y, float z) {
	TFX_ASSERT(tfx__valid_effect_id(pm, effect_index));    //Not a valid effect id. Make sure that when you call tfx_AddEffectTemplateToParticleManager you check that it returns true.
	pm->effects[effect_index].local_position += {x, y, z};
}

tfxAPI void tfx_GetEffectPositionVec3(tfx_particle_manager pm, tfxEffectID effect_index, float out_position[3]) {
	TFX_ASSERT(tfx__valid_effect_id(pm, effect_index));    //Not a valid effect id. Make sure that when you call tfx_AddEffectTemplateToParticleManager you check that it returns true.
...
tfxAPI tfx_2d_instance_t *tfx_GetEffect2dInstanceBuffer(tfx_particle_manager pm, tfxEffectID effect_index, tfxU32 *sprite_count) {
	TFX_ASSERT(tfx__valid_effect_id(pm, effect_index));    //Not a valid effect id. Make sure that when you call tfx_AddEffectTemplateToParticleManager you check that it returns true.
...
tfxAPI tfx_3d_instance_t *tfx_GetEffect3dInstanceBuffer(tfx_particle_manager pm, tfxEffectID effect_index, tfxU32 *sprite_count) {
	TFX_ASSERT(tfx__valid_effect_id(pm, effect_index));    //Not a valid effect id. Make sure that when you call tfx_AddEffectTemplateToParticleManager you check that it returns true.
	tfx_effect_instance_data_t &instance_data = pm->effects[effect_index].instance_data;
	*sprite_count = instance_data.instance_count;
...
bool tfx_GetNext2dInstanceBuffer(tfx_particle_manager pm, tfx_2d_instance_t **instances, tfx_effect_instance_data_t **instance_data, tfxU32 *instance_count) {
...
bool tfx_GetNext3dInstanceBuffer(tfx_particle_manager pm, tfx_3d_instance_t **instances, tfx_effect_instance_data_t **instance_data, tfxU32 *instance_count) {
	if (pm->effect_index_position >= pm->effects_in_use[0][pm->current_ebuff].current_size) {
		*instances = nullptr;
		*instance_data = nullptr;
		*instance_count = 0;
...
void tfx_ResetInstanceBufferLoopIndex(tfx_particle_manager pm) {
	pm->effect_index_position = 0;
}

void tfx_SetEffect2dRotation(tfx_particle_manager pm, tfxEffectID effect_index, float rotation) {
	TFX_ASSERT(tfx__valid_effect_id(pm, effect_index));    //Not a valid effect id. Make sure that when you call tfx_AddEffectTemplateToParticleManager you check that it returns true.
...
void tfx_SetEffectRoll(tfx_particle_manager pm, tfxEffectID effect_index, float roll) {
	TFX_ASSERT(tfx__valid_effect_id(pm, effect_index));    //Not a valid effect id. Make sure that when you call tfx_AddEffectTemplateToParticleManager you check that it returns true.
	pm->effects[effect_index].local_rotations.roll = roll;
	pm->effects[effect_index].state_flags |= tfxEffectStateFlags_override_orientiation;
}

void tfx_SetEffectPitch(tfx_particle_manager pm, tfxEffectID effect_index, float pitch) {
	TFX_ASSERT(tfx__valid_effect_id(pm, effect_index));    //Not a valid effect id. Make sure that when you call tfx_AddEffectTemplateToParticleManager you check that it returns true.
	pm->effects[effect_index].local_rotations.pitch = pitch;
	pm->effects[effect_index].state_flags |= tfxEffectStateFlags_override_orientiation;
}

void tfx_SetEffectYaw(tfx_particle_manager pm, tfxEffectID effect_index, float pitch) {
	TFX_ASSERT(tfx__valid_effect_id(pm, effect_index));    //Not a valid effect id. Make sure that when you call tfx_AddEffectTemplateToParticleManager you check that it returns true.
	pm->effects[effect_index].local_rotations.pitch = pitch;
	pm->effects[effect_index].state_flags |= tfxEffectStateFlags_override_orientiation;
}

void tfx_SetEffectWidthMultiplier(tfx_particle_manager pm, tfxEffectID effect_index, float width) {
	TFX_ASSERT(tfx__valid_effect_id(pm, effect_index));    //Not a valid effect id. Make sure that when you call tfx_AddEffectTemplateToParticleManager you check that it returns true.
...
void tfx_SetEffectHeightMultiplier(tfx_particle_manager pm, tfxEffectID effect_index, float height) {
	TFX_ASSERT(tfx__valid_effect_id(pm, effect_index));    //Not a valid effect id. Make sure that when you call tfx_AddEffectTemplateToParticleManager you check that it returns true.
	pm->effects[effect_index].emitter_size.y = height;
	pm->effects[effect_index].state_flags |= tfxEffectStateFlags_override_size_multiplier;
}

void tfx_SetEffectDepthMultiplier(tfx_particle_manager pm, tfxEffectID effect_index, float depth) {
	TFX_ASSERT(tfx__valid_effect_id(pm, effect_index));    //Not a valid effect id. Make sure that when you call tfx_AddEffectTemplateToParticleManager you check that it returns true.
	pm->effects[effect_index].emitter_size.z = depth;
	pm->effects[effect_index].state_flags |= tfxEffectStateFlags_override_size_multiplier;
}

void tfx_SetEffectLifeMultiplier(tfx_particle_manager pm, tfxEffectID effect_index, float life) {
	TFX_ASSERT(tfx__valid_effect_id(pm, effect_index));    //Not a valid effect id. Make sure that when you call tfx_AddEffectTemplateToParticleManager you check that it returns true.
	pm->effects[effect_index].spawn_controls.life = life;
}

void tfx_SetEffectParticleWidthMultiplier(tfx_particle_manager pm, tfxEffectID effect_index, float width) {
	TFX_ASSERT(tfx__valid_effect_id(pm, effect_index));    //Not a valid effect id. Make sure that when you call tfx_AddEffectTemplateToParticleManager you check that it returns true.
	pm->effects[effect_index].spawn_controls.size_x = width;
}

void tfx_SetEffectParticleHeightMultiplier(tfx_particle_manager pm, tfxEffectID effect_index, float height) {
	TFX_ASSERT(tfx__valid_effect_id(pm, effect_index));    //Not a valid effect id. Make sure that when you call tfx_AddEffectTemplateToParticleManager you check that it returns true.
	pm->effects[effect_index].spawn_controls.size_y = height;
}

void tfx_SetEffectVelocityMultiplier(tfx_particle_manager pm, tfxEffectID effect_index, float velocity) {
	TFX_ASSERT(tfx__valid_effect_id(pm, effect_index));    //Not a valid effect id. Make sure that when you call tfx_AddEffectTemplateToParticleManager you check that it returns true.
	pm->effects[effect_index].spawn_controls.velocity = velocity;
}

void tfx_SetEffectSpinMultiplier(tfx_particle_manager pm, tfxEffectID effect_index, float spin) {
	TFX_ASSERT(tfx__valid_effect_id(pm, effect_index));    //Not a valid effect id. Make sure that when you call tfx_AddEffectTemplateToParticleManager you check that it returns true.
	pm->effects[effect_index].spawn_controls.spin = spin;
}

void tfx_SetEffectIntensityMultiplier(tfx_particle_manager pm, tfxEffectID effect_index, float intensity) {
	TFX_ASSERT(tfx__valid_effect_id(pm, effect_index));    //Not a valid effect id. Make sure that when you call tfx_AddEffectTemplateToParticleManager you check that it returns true.
	pm->effects[effect_index].spawn_controls.intensity = intensity;
}

void tfx_SetEffectSplatterMultiplier(tfx_particle_manager pm, tfxEffectID effect_index, float splatter) {
	TFX_ASSERT(tfx__valid_effect_id(pm, effect_index));    //Not a valid effect id. Make sure that when you call tfx_AddEffectTemplateToParticleManager you check that it returns true.
	pm->effects[effect_index].spawn_controls.splatter = splatter;
}

void tfx_SetEffectWeightMultiplier(tfx_particle_manager pm, tfxEffectID effect_index, float weight) {
	TFX_ASSERT(tfx__valid_effect_id(pm, effect_index));    //Not a valid effect id. Make sure that when you call tfx_AddEffectTemplateToParticleManager you check that it returns true.
	pm->effects[effect_index].spawn_controls.weight = weight;
}

void tfx_SetEffectOveralScale(tfx_particle_manager pm, tfxEffectID effect_index, float overal_scale) {
	TFX_ASSERT(tfx__valid_effect_id(pm, effect_index));    //Not a valid effect id. Make sure that when you call tfx_AddEffectTemplateToParticleManager you check that it returns true.
...
void tfx_SetEffectBaseNoiseOffset(tfx_particle_manager pm, tfxEffectID effect_index, float noise_offset) {
	TFX_ASSERT(tfx__valid_effect_id(pm, effect_index));    //Not a valid effect id. Make sure that when you call tfx_AddEffectTemplateToParticleManager you check that it returns true.
