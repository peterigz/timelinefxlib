 ...
 */
	...
	*/
	...
	float GetEmissionDirection2d(tfxCommon &common, tfxEmitterState &current, tfxEffectEmitter *library_link, tfxVec2 local_position, tfxVec2 world_position, tfxVec2 emitter_size) {
		...
		float direction = 0;
		tfxEmissionType emission_type = common.library->emitter_properties.emission_type[library_link->property_index];
		tfxEmissionDirection emission_direction = common.library->emitter_properties.emission_direction[library_link->property_index];
	...
	void InitialiseParticle2d(tfxParticleData &data, tfxSpriteTransform2d &sprite_transform, tfxEmitterState &current, tfxCommon &common, tfxEmitterSpawnControls &spawn_values, tfxEffectEmitter *library_link, float tween) {
		...
		tfxEmissionType emission_type = common.library->emitter_properties.emission_type[library_link->property_index];
		tfxVec3 grid_points = common.library->emitter_properties.grid_points[library_link->property_index];
		tfxImageData *image = common.library->emitter_properties.image[library_link->property_index];
		tfxAngleSettingFlags angle_settings = common.library->emitter_properties.angle_settings[library_link->property_index];
		tfxVec3 angle_offsets = common.library->emitter_properties.angle_offsets[library_link->property_index];
		float start_frame = common.library->emitter_properties.start_frame[library_link->property_index];
		...
		if (angle_settings & tfxAngleSettingFlags_align_roll && common.property_flags & tfxEmitterPropertyFlags_edge_traversal)
			sprite_transform.rotation = data.local_rotations.roll = direction + angle_offsets.roll;

		bool line = common.property_flags & tfxEmitterPropertyFlags_edge_traversal && emission_type == tfxLine;
	...
	tfxSpawnPosition InitialisePosition3d(tfxEmitterState &current, tfxCommon &common, tfxEmitterSpawnControls &spawn_values, tfxEffectEmitter *library_link, float tween) {
		...
		tfxEmissionType emission_type = common.library->emitter_properties.emission_type[library_link->property_index];
		tfxVec3 grid_points = common.library->emitter_properties.grid_points[library_link->property_index];
		tfxVectorAlignType vector_align_type = common.library->emitter_properties.vector_align_type[library_link->property_index];
			...
			if (common.property_flags & tfxEmitterPropertyFlags_spawn_on_grid) {
				...
				if (common.property_flags & tfxEmitterPropertyFlags_fill_area) {
					...
					if (common.property_flags & tfxEmitterPropertyFlags_grid_spawn_random) {
						...
						tfxU32 side = random_generation.RangeUInt((common.property_flags & tfxEmitterPropertyFlags_area_open_ends) ? 4 : 6);
						if (side == 0) {
							//left side
							current.grid_coords.x = 0.f;
							current.grid_coords.y = (float)random_generation.RangeUInt((tfxU32)grid_points.y);
							...
							if (current.grid_direction.z == 0) {
								...
								if (current.grid_coords.z < 0.f) {
									...
									if (current.grid_coords.x < 0) {
										...
										current.grid_direction.z = (common.property_flags & tfxEmitterPropertyFlags_area_open_ends) ? 0.f : 4.f;
									}
								}
							}
		...
		else if (emission_type == tfxIcosphere && !(common.property_flags & tfxEmitterPropertyFlags_grid_spawn_random)) {
			tfxVec3 emitter_size = current.emitter_size * .5f;
		...
		else if (common.property_flags & tfxEmitterPropertyFlags_edge_traversal && emission_type == tfxLine) {
	...
	void tfxParticleManager::AddEffect(tfxEffectEmitter &effect, unsigned int buffer, bool is_sub_emitter, float add_delayed_spawning) {
		...
		for (auto &e : effect.GetInfo().sub_effectors) {
			...
			if (e.flags & tfxEmitterStateFlags_enabled) {
				...
				emitter.common.delay_spawning = properties.delay_spawning[emitter.property_index];
...
return free_slot;
				...
				if (!(p.data.flags & tfxParticleFlags_fresh)) {
					...
					if (p.parent && ControlParticle(pm, p, s.transform.scale, *p.parent)) {
						...
						if (p.parent->common.property_flags & tfxEmitterPropertyFlags_relative_position && properties.vector_align_type[property_index] == tfxVectorAlignType_emission)
							...
							alignment_vector = SetParticleAlignment(p, s.transform.position, properties.vector_align_type[property_index]);
						p.next_ptr = pm.SetNextParticle(next_buffer_index, p);
	...
	void tfxParticleManager::ClearAll(bool free_memory) {
		...
		if (flags & tfxEffectManagerFlags_unordered) {
			if (free_memory) {
				...
				for (auto &bank : particle_arrays) {
					bank.parent.clear();
					bank.sprite_index.clear();
					bank.next_id.clear();
					bank.flags.clear();
					bank.age.clear();
					bank.max_age.clear();
					bank.local_position.clear();
					bank.captured_position.clear();
					bank.local_rotations.clear();
					bank.velocity_normal.clear();
					bank.stretch.clear();
					bank.weight_acceleration.clear();
					bank.base_weight.clear();
					bank.base_velocity.clear();
					bank.base_spin.clear();
					bank.noise_offset.clear();
					bank.noise_resolution.clear();
					bank.color.clear();
					bank.intensity.clear();
					bank.image_frame.clear();
					bank.base_size.clear();
					bank.single_loop_count.clear();
				}
			}
	...
	void tfxParticleManager::FreeParticleBanks() {
		...
		for (auto &bank : particle_arrays) {
			bank.parent.free_all();
			bank.sprite_index.free_all();
			bank.next_id.free_all();
			bank.flags.free_all();
			bank.age.free_all();
			bank.max_age.free_all();
			bank.local_position.free_all();
			bank.captured_position.free_all();
			bank.local_rotations.free_all();
			bank.velocity_normal.free_all();
			bank.stretch.free_all();
			bank.weight_acceleration.free_all();
			bank.base_weight.free_all();
			bank.base_velocity.free_all();
			bank.base_spin.free_all();
			bank.noise_offset.free_all();
			bank.noise_resolution.free_all();
			bank.color.free_all();
			bank.intensity.free_all();
			bank.image_frame.free_all();
			bank.base_size.free_all();
			bank.single_loop_count.free_all();
		}
	...
	tfxU32 GrabParticleLists(tfxParticleManager &pm, tfxKey emitter_hash, tfxU32 reserve_amount) {
		...
		lists.parent.reserve(reserve_amount);
		lists.age.reserve(reserve_amount);
		lists.flags.reserve(reserve_amount);
		lists.max_age.reserve(reserve_amount);
		lists.local_position.reserve(reserve_amount);
		lists.captured_position.reserve(reserve_amount);
		lists.local_rotations.reserve(reserve_amount);
		lists.velocity_normal.reserve(reserve_amount);
		lists.stretch.reserve(reserve_amount);
		lists.weight_acceleration.reserve(reserve_amount);
		lists.base_weight.reserve(reserve_amount);
		lists.base_velocity.reserve(reserve_amount);
		lists.base_spin.reserve(reserve_amount);
		lists.noise_offset.reserve(reserve_amount);	
		lists.noise_resolution.reserve(reserve_amount);
		lists.color.reserve(reserve_amount);	
		lists.intensity.reserve(reserve_amount);
		lists.image_frame.reserve(reserve_amount);
		lists.base_size.reserve(reserve_amount);
		lists.single_loop_count.reserve(reserve_amount);			

		return pm.particle_arrays.ts_push_back(lists);
		//pm.particle_arrays.push_back(lists);
		//return pm.particle_arrays.current_size - 1;
	}
	...
	void UpdatePMEmitter(tfxParticleManager &pm, tfxSpawnWorkEntry *spawn_work_entry) {
		...
		if (e.parent && e.parent->type != tfxFolder && e.parent->next_ptr) {
			...
			spawn_work_entry->spawn_controls = UpdateEmitterState(e, pm.parent_spawn_controls);
			...
			if (next_id != tfxINVALID) {
				...
				e.common.transform.world_position += properties.emitter_handle[e.property_index] * e.current.overal_scale;
				if (e.flags & tfxEmitterStateFlags_no_tween_this_update || e.flags & tfxEmitterStateFlags_no_tween) {
					e.common.transform.captured_position = e.common.transform.world_position;
				}
			}
		...
		if (!(e.common.property_flags & tfxEmitterPropertyFlags_single) || properties.single_shot_limit[e.property_index] > 0)
			e.highest_particle_age -= tfxFRAME_LENGTH;

		if (properties.loop_length && e.common.age > properties.loop_length[e.property_index])
			e.common.age -= properties.loop_length[e.property_index];
	...
	void SpawnParticleAge(tfxWorkQueue *queue, void *data) {
		...
		for(int i = 0; i != entry->amount_to_spawn; ++i) {
			...
			assert(e.sprites_index < pm.sprites2d[properties.layer[e.property_index]].capacity);
			tfxU32 &sprite_index = pm.GrabSpriteIndex(e.particles_index);
			tfxU32 &next_id = pm.GrabNextIndex(e.particles_index);
			tfxParticleFlags &flags = pm.GrabFlags(e.particles_index);
	...
	void SpawnParticlePositions2d(tfxWorkQueue *queue, void *data) {
		...
		for(int i = 0; i != entry->amount_to_spawn; ++i) {
			...
			tfxVec3 &velocity_normal = pm.GrabVelocityNormal(e.particles_index);
			tfxVec3 &local_rotations = pm.GrabLocalRotations(e.particles_index);
			...
			//----Position
			local_position = 0;
			tfxVec2 lerp_position = InterpolateVec2(tween, e.common.transform.captured_position.xy(), e.common.transform.world_position.xy());
			if (emission_type == tfxPoint) {
				if (e.common.property_flags & tfxEmitterPropertyFlags_relative_position)
					...
					if (e.common.property_flags & tfxEmitterPropertyFlags_emitter_handle_auto_center) {
						...
						tfxVec2 rotvec = mmTransformVector(e.common.transform.matrix, -e.common.handle.xy());
				...
				//----TForm and Emission
				if (!(e.common.property_flags & tfxEmitterPropertyFlags_relative_position)) {
					local_position = mmTransformVector(e.common.transform.matrix, local_position.xy() + e.common.handle.xy());
					local_position = lerp_position + local_position.xy() * e.common.transform.scale.xy();
				}
	...
	tfxEmitterSpawnControls UpdateEmitterState(tfxEffectEmitter &e, tfxParentSpawnControls &parent_spawn_controls) {
		...
		e.common.transform.translation.x = lookup_callback(e.common.library->transform_attributes[e.transform_attributes].translation_x, e.common.frame);
		e.common.transform.translation.y = lookup_callback(e.common.library->transform_attributes[e.transform_attributes].translation_y, e.common.frame);
		e.common.transform.translation.z = lookup_callback(e.common.library->transform_attributes[e.transform_attributes].translation_z, e.common.frame);
		e.current.overal_scale = parent.current.overal_scale;
		e.common.transform.local_rotations.roll = LookupPrecise(e.common.library->transform_attributes[e.transform_attributes].roll, e.common.age);
		e.common.transform.local_rotations.pitch = LookupPrecise(e.common.library->transform_attributes[e.transform_attributes].pitch, e.common.age);
		e.common.transform.local_rotations.yaw = LookupPrecise(e.common.library->transform_attributes[e.transform_attributes].yaw, e.common.age);
		...
		if (e.common.property_flags & tfxEmitterPropertyFlags_emitter_handle_auto_center && properties.emission_type[e.property_index] != tfxPoint) {
			...
			e.common.handle = properties.emitter_handle[e.property_index];
		}
		else {
			e.common.handle = 0.f;
		}
	...
	void ControlParticleAge(tfxWorkQueue *queue, void *data) {
		...
		tfxParticleArrays &bank = work_entry->pm->particle_arrays[e.particles_index];
		tfxU32 single_shot_limit = e.GetProperties().single_shot_limit[e.property_index];

		tfxU32 offset = 0;
		for (int i = work_entry->start_index; i >= 0; --i) {
			float &age = bank.age[i];
			const float &max_age = bank.max_age[i];
			tfxU32 &single_loop_count = bank.single_loop_count[i];
			tfxParticleID &next_index = bank.next_id[i];
			...
			if (flags & tfxParticleFlags_remove) {
				...
				bank.local_position[i + offset] = bank.local_position[i];
				bank.captured_position[i + offset] = bank.captured_position[i];
				bank.local_rotations[i + offset] = bank.local_rotations[i];
				bank.velocity_normal[i + offset] = bank.velocity_normal[i];
				bank.stretch[i + offset] = bank.stretch[i];
				bank.weight_acceleration[i + offset] = bank.weight_acceleration[i];
				bank.base_weight[i + offset] = bank.base_weight[i];
				bank.base_velocity[i + offset] = bank.base_velocity[i];
				bank.base_spin[i + offset] = bank.base_spin[i];
				bank.noise_offset[i + offset] = bank.noise_offset[i];		
				bank.noise_resolution[i + offset] = bank.noise_resolution[i];
				bank.color[i + offset] = bank.color[i];			
				bank.intensity[i + offset] = bank.intensity[i];
				bank.image_frame[i + offset] = bank.image_frame[i];
				bank.base_size[i + offset] = bank.base_size[i];
				bank.single_loop_count[i + offset] = bank.single_loop_count[i];	
			}

		}

		if (offset) {
			bank.parent.bump(offset);
			bank.sprite_index.bump(offset);
			bank.next_id.bump(offset);
			bank.flags.bump(offset);
			bank.age.bump(offset);
			bank.max_age.bump(offset);
			bank.local_position.bump(offset);
			bank.captured_position.bump(offset);
			bank.local_rotations.bump(offset);
			bank.velocity_normal.bump(offset);
			bank.stretch.bump(offset);
			bank.weight_acceleration.bump(offset);
			bank.base_weight.bump(offset);
			bank.base_velocity.bump(offset);
			bank.base_spin.bump(offset);
			bank.noise_offset.bump(offset);
			bank.noise_resolution.bump(offset);
			bank.color.bump(offset);
			bank.intensity.bump(offset);
			bank.image_frame.bump(offset);
			bank.base_size.bump(offset);
			bank.single_loop_count.bump(offset);
		}

	}

	void ControlParticlePosition(tfxWorkQueue *queue, void *data) {
		tfxControlWorkEntry *work_entry_ptr = static_cast<tfxControlWorkEntry*>(data);
		tfxControlWorkEntry work_entry = *work_entry_ptr;
		...
		for (int i = work_entry.start_index; i >= 0; --i) {
			...
			const float base_weight = bank.base_weight[i];
			const float base_velocity = bank.base_velocity[i];
			const float base_spin = bank.base_spin[i];
			const float noise_offset = bank.noise_offset[i];
			const float noise_resolution = bank.noise_resolution[i];
			const float angle = bank.velocity_normal[i].x;
			float &weight_acceleration = bank.weight_acceleration[i];
		...
		for (int i = work_entry.start_index; i >= 0; --i) {
			const tfxVec3 &local_position = bank.local_position[i];
			const tfxVec3 &local_rotations = bank.local_rotations[i];
			tfxVec3 &captured_position = bank.captured_position[i];
			tfxParticleFlags &flags = bank.flags[i];
	...
	void ControlParticleSize(tfxWorkQueue *queue, void *data) {
		...
		for (int i = work_entry.start_index; i >= 0; --i) {
			...
			const tfxVec2 base_size = bank.base_size[i];
			const float base_velocity = bank.base_velocity[i];
			const float weight_acceleration = bank.weight_acceleration[i];
	...
	void ControlParticleColor(tfxWorkQueue *queue, void *data) {
		...
		for (int i = work_entry.start_index; i >= 0; --i) {
			...
			tfxRGBA8 &color = bank.color[i];
			float &intensity = bank.intensity[i];
	...
	void ControlParticleImageFrame(tfxWorkQueue *queue, void *data) {
		...
		for (int i = work_entry.start_index; i >= 0; --i) {
			...
			float &image_frame = bank.image_frame[i];
			tfxU32 &sprites_index = bank.sprite_index[i];
	...
	void ControlParticles2d(tfxParticleManager &pm, tfxEffectEmitter &e, tfxControlWorkEntry &work_entry) {
		...
		tfxring<float> &bank = pm.particle_arrays[e.particles_index].age;
		int offset = 0;
