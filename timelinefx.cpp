...
void tfx__update_effect_max_life(tfx_effect_descriptor_t *effect) {
...
void tfx__initialise_unitialised_graphs(tfx_effect_descriptor_t *effect) {
	...
	if (effect->type == tfxRibbonType) {
		...
		if (library->graphs[graph_list_index].graphs[tfxRibbon_factor_intensity_index].nodes.size() == 0) tfx__reset_graph(&library->graphs[graph_list_index].graphs[tfxRibbon_factor_intensity_index], 1.f, tfxPercentOvertime);
	}
}
...
void tfx__assign_effector_property_bool(tfx_effect_descriptor_t *effect, tfx_str256_t *field, bool value) {
	...
	if (*field == "is_3d") {
		if (value) { effect->shared_flags |= tfxSharedEmitterPropertyFlags_effect_is_3d; effect->effect_flags |= tfxEffectPropertyFlags_effect_is_3d; }
		else { effect->shared_flags &= ~tfxSharedEmitterPropertyFlags_effect_is_3d; effect->effect_flags |= tfxEffectPropertyFlags_effect_is_3d; }
	}
...
void tfx__copy_graph_color(tfx_graph_list_t *from, tfx_graph_list_t *to) {
	TFX_ASSERT(from->effect_descriptor_type == to->effect_descriptor_type);
	if (from->effect_descriptor_type == tfxEmitterType) {
		tfx__clear_graph(&to->graphs[tfxEmitter_overtime_red_index]);
		tfx__clear_graph(&to->graphs[tfxEmitter_overtime_green_index]);
		tfx__clear_graph(&to->graphs[tfxEmitter_overtime_blue_index]);
		for (tfxBucketLoop(from->graphs[tfxEmitter_overtime_red_index].nodes, i)) {
			to->graphs[tfxEmitter_overtime_red_index].nodes.push_back(from->graphs[tfxEmitter_overtime_red_index].nodes[i]);
		}
		for (tfxBucketLoop(from->graphs[tfxEmitter_overtime_green_index].nodes, i)) {
			to->graphs[tfxEmitter_overtime_green_index].nodes.push_back(from->graphs[tfxEmitter_overtime_green_index].nodes[i]);
		}
		for (tfxBucketLoop(from->graphs[tfxEmitter_overtime_blue_index].nodes, i)) {
			to->graphs[tfxEmitter_overtime_blue_index].nodes.push_back(from->graphs[tfxEmitter_overtime_blue_index].nodes[i]);
		}
	} else if(from->effect_descriptor_type == tfxRibbonType) {
		tfx__clear_graph(&to->graphs[tfxRibbon_overtime_red_index]);
		tfx__clear_graph(&to->graphs[tfxRibbon_overtime_green_index]);
		tfx__clear_graph(&to->graphs[tfxRibbon_overtime_blue_index]);
		for (tfxBucketLoop(from->graphs[tfxRibbon_overtime_red_index].nodes, i)) {
			to->graphs[tfxRibbon_overtime_red_index].nodes.push_back(from->graphs[tfxRibbon_overtime_red_index].nodes[i]);
		}
		for (tfxBucketLoop(from->graphs[tfxRibbon_overtime_green_index].nodes, i)) {
			to->graphs[tfxRibbon_overtime_green_index].nodes.push_back(from->graphs[tfxRibbon_overtime_green_index].nodes[i]);
		}
		for (tfxBucketLoop(from->graphs[tfxRibbon_overtime_blue_index].nodes, i)) {
			to->graphs[tfxRibbon_overtime_blue_index].nodes.push_back(from->graphs[tfxRibbon_overtime_blue_index].nodes[i]);
		}
	}
}
...
float tfx__get_max_life(tfx_effect_descriptor_t *e) {
	...
	if (life_last_frame + life_variation_last_frame > 0) {
		for (float f = 0; f < fmaxf(life_last_frame, life_variation_last_frame); ++f) {
			if (e->parent) {
				global_adjust = tfx__get_graph_value_by_age(&e->parent->library->graphs[e->parent->graph_list_index].graphs[tfxEffect_global_life_index], f);
			}
...
tfxEffectID tfx__add_effect_to_particle_manager(tfx_particle_manager pm, tfx_effect_descriptor_t *effect, int buffer, int hierarchy_depth, bool is_sub_emitter, tfxU32 root_effect_index, float add_delayed_spawning) {
	...
	tfx_particle_emitter_properties_t *properties = tfx__get_particle_emitter_properties(effect);
	...
	for (auto &e : tfx_GetEffectInfo(effect)->sub_effectors) {
		if (e.shared_flags & tfxSharedEmitterPropertyFlags_enabled) {
			...
			if (e.type == tfxEmitterType) {
				...
				ribbon_emitter.segment_count = ribbon_properties->segment_count;
				...
				tfxU32 bucket_index = tfxRibbonBucketIndex(ribbon_emitter.segment_count);
				if (!(pm->ribbon_segment_buckets[bucket_index].flags & tfxRibbonBucketFlags_initialised)) {
					tfx__init_ribbon_segment_buffer(pm, bucket_index, 1);
				}
				pm->ribbon_segment_buckets[bucket_index].ribbon_emitter_indexes[pm->current_ebuff].push_back(index);
				...
				if (shared_properties->emission_type == tfxPath) {
					...
					path_state.path_cycle_count = path->maximum_paths;
					path_state.path_start_index = 0;
					ribbon_emitter.ribbon_segments_index = tfx__grab_ribbon_segment_lists(pm, e.path_hash, (effect->shared_flags & tfxSharedEmitterPropertyFlags_effect_is_3d), path->maximum_active_paths * ribbon_properties->segment_count, e.control_profile);
				}
			}
...
void tfx_UpdateParticleManager(tfx_particle_manager pm, float elapsed_time) {
	...
	while (tfx__next_ribbon_emitter(pm, &ribbon_dispatch)) {
		...
		tfxU32 bucket_index = tfxRibbonBucketIndex(ribbon_dispatch.ribbon_data->globals.segment_count);
		tfxUnFlagRibbonBucketInUse(pm->ribbon_buckets_in_use, bucket_index);
		for (int ribbon_index : bucket.ribbon_emitter_indexes[pm->current_ebuff]) {
			...
			tfx_library library = pm->ribbon_emitters[ribbon_index].library;
			ribbon_work_entry->properties = &library->ribbon_properties[pm->ribbon_emitters[ribbon_index].properties_index];
			ribbon_work_entry->shared_properties = &library->shared_properties[pm->ribbon_emitters[ribbon_index].shared_index];
			...
			if (timeout_counter <= pm->ribbon_emitters[ribbon_index].timeout) {
				bucket.ribbon_emitter_indexes[next_buffer].push_back(ribbon_index);
	...
	bool is_recording = (pm->flags & tfxParticleManagerFlags_recording_sprites) > 0 && (pm->flags & tfxParticleManagerFlags_using_uids) > 0;
	{
		...
		for (int index : pm->control_ribbon_queue) {
			tfx_ribbon_emitter_state_t &ribbon_emitter = pm->ribbon_emitters[index];
...

	{
		...
		for (int index : pm->control_ribbon_queue) {
			...
			if (ribbon_emitter.active_ribbons > 0) {
				...
				work_entry.ribbon_index = index;
				work_entry.pm = pm;
				tfx__add_work_queue_entry(&pm->work_queue, &work_entry, tfx__control_ribbon_path_age);
			}
		}
...
void tfx__control_ribbons(tfx_work_queue_t *queue, void *data) {
	...
	tfx_ribbon_emitter_properties_t &properties = library->ribbon_properties[ribbon_emitter.properties_index];
...
void tfx__control_ribbon_attributes(tfx_work_queue_t *queue, void *data) {
	...
	tfx_ribbon_bucket_t &bucket = pm.ribbon_segment_buckets[tfxRibbonBucketIndex(ribbon_emitter.segment_count)];
...
void tfx__control_ribbon_path_age(tfx_work_queue_t *queue, void *data) {
	...
	tfx_random_t &random = work_entry->random;
	...
	for (tfxU32 ribbon_index : ribbon_emitter.ribbon_indexes[pm.current_ebuff]) {
		...
		bucket.highest_segment_index = tfx__Max(bucket.highest_segment_index, bucket.ribbons.ribbon_instances->start_index + ribbon_emitter.segment_count);
		bucket.lowest_segment_index = tfx__Min(bucket.lowest_segment_index, bucket.ribbons.ribbon_instances->start_index);
	}

	if (bucket.highest_ribbon_index >= bucket.lowest_ribbon_index) {
		pm.flags |= tfxParticleManagerFlags_has_ribbons_to_draw;
		tfxFlagRibbonBucketInUse(pm.ribbon_buckets_in_use, tfxRibbonBucketIndex(ribbon_emitter.segment_count));
...
void tfx__control_ribbon_paths(tfx_work_queue_t *queue, void *data) {
	...
	tfx_emitter_path_t *path = &library->paths[ribbon_emitter.path_attributes];
	tfxWideFloat node_count = tfxWideSetSingle((float)path->node_count - 3.f);
	tfx_vector_t<tfx_ribbon_segment_t> &segments = pm.ribbon_segment_buckets[tfxRibbonBucketIndex(ribbon_emitter.segment_count)].segments;
...
tfx_ribbon_bucket_t *tfx_Get3dRibbonBuffers(tfx_particle_manager pm, tfxU32 segment_count) {
	tfx_ribbon_bucket_t *bucket = nullptr;
	tfxU32 index = tfxRibbonBucketIndex(segment_count);
	if (pm->ribbon_segment_buckets[index].flags & tfxRibbonBucketFlags_initialised) {
		bucket = &pm->ribbon_segment_buckets[index];
	}
	return bucket;
}
...
tfx_ribbon_buffer_info_t tfx_GetRibbonBufferInfo(tfx_particle_manager pm, tfxU32 segment_count) {
	tfx_ribbon_buffer_info_t info{};
	tfxU32 index = tfxRibbonBucketIndex(segment_count);
	if (pm->ribbon_segment_buckets[index].flags & tfxRibbonBucketFlags_initialised) {
		info = pm->ribbon_segment_buckets[index].buffer_info;
	}
	return info;
}
...
void tfx__reset_ribbon_buffer_requirements(tfx_particle_manager pm) {
	...
	int index = tfx__scan_forward(pm->ribbon_dispatches);
	while (index != -1) {
		tfx_ribbon_bucket_t *bucket = &pm->ribbon_segment_buckets[index];
		tfxUnFlagRibbonBucketInUse(pm->ribbon_dispatches, index);
		...
		index = tfx__scan_forward(pm->ribbon_dispatches);
	}
	pm->ribbon_dispatches = pm->ribbon_buckets_in_use;
}
...
bool tfx__next_ribbon_emitter(tfx_particle_manager pm, tfx_ribbon_dispatch_t *ribbon_dispatch) {
	int index = tfx__scan_forward(pm->ribbon_dispatches);
	while (index != -1) {
		tfx_ribbon_bucket_t *bucket = &pm->ribbon_segment_buckets[index];
		tfxUnFlagRibbonBucketInUse(pm->ribbon_dispatches, index);
...
bool tfx_NextRibbonDispatch(tfx_particle_manager pm, tfx_ribbon_dispatch_t *ribbon_dispatch) {
	...
	while (index != -1) {
		...
		ribbon_dispatch->index_count = (ribbon_count * bucket->globals.segment_count) * bucket->buffer_info.indices_per_segment - (ribbon_count * bucket->buffer_info.indices_per_segment);
		...
		bucket->globals.segment_offset = ribbon_dispatch->segment_offset;
		bucket->globals.ribbon_count = ribbon_count;
		pm->last_ribbon_dispatch = *ribbon_dispatch;
		pm->last_ribbon_dispatch.index_offset += bucket->buffer_info.index_count;
...
void tfx_CopyRibbonDataToStagingBuffers(tfx_particle_manager pm, void *segments_dst, void *ribbons_dst, void *emitters_dst) {
	...
	if (segments_dst && ribbons_dst) {
		...
		while (index != -1) {
			...
			running_segment_offset += ribbon_count * bucket->globals.segment_count;
...
void tfx_ClearParticleManager(tfx_particle_manager pm, bool free_particle_banks, bool free_sprite_buffers) {
	...
	for (tfx_ribbon_bucket_t &ribbon_bucket : pm->ribbon_segment_buckets) {
		...
		ribbon_bucket.flags = (ribbon_bucket.flags & tfxRibbonBucketFlags_initialised) ? tfxRibbonBucketFlags_initialised : 0;
	}
	pm->gpu_emitters.clear();
	pm->cached_static_path_segments.Clear();
	pm->ribbon_buckets_in_use = 0;
...
void tfx_FreeParticleManager(tfx_particle_manager pm) {
	...
	for (int i = 0; i != tfxArraySize_segment_bucket; ++i) {
		pm->ribbon_segment_buckets[i].free_ribbons.free();
		tfx__free_soa_buffer(&pm->ribbon_segment_buckets[i].ribbons_buffer);
		pm->ribbon_segment_buckets[i].segments.free();
		pm->ribbon_segment_buckets[i].ribbon_emitter_indexes[0].free();
		pm->ribbon_segment_buckets[i].ribbon_emitter_indexes[1].free();
	}
...
tfxU32 tfx__grab_ribbon(tfx_particle_manager pm, tfx_ribbon_emitter_state_t *ribbon_emitter) {
	tfxU32 index = tfxRibbonBucketIndex(ribbon_emitter->segment_count);
...
void tfx__free_ribbon(tfx_particle_manager pm, tfxU32 segment_count, tfxU32 ribbon_index) {
	tfxU32 index = tfxRibbonBucketIndex(segment_count);
	TFX_ASSERT(pm->ribbon_segment_buckets[index].flags & tfxRibbonBucketFlags_initialised);
	tfx_ribbon_bucket_t &bucket = pm->ribbon_segment_buckets[index];
...
void tfx__init_ribbon_segment_buffer(tfx_particle_manager pm, tfxU32 index, int tessellation) {
	tfx_ribbon_bucket_t &bucket = pm->ribbon_segment_buckets[index];
	if (bucket.flags & tfxRibbonBucketFlags_initialised) {
		tfx__clear_soa_buffer(&bucket.ribbons_buffer);
	} else {
		...
		bucket.globals.segment_count = (index + 1) * tfxMIN_SEGMENT_COUNT;
	...
	if (effect.parent_particle_index != tfxINVALID) {
		...
		if (parent_particle_id != tfxINVALID) {
			...
			if (sprite_id != tfxINVALID) {
				if (effect.effect_flags & tfxEffectPropertyFlags_effect_is_3d) {
...
void tfx__update_ribbon_emitter(tfx_work_queue_t *work_queue, void *data) {
	tfx_ribbon_work_entry_t *ribbon_work_entry = static_cast<tfx_ribbon_work_entry_t *>(data);
	tfxU32 emitter_index = ribbon_work_entry->ribbon_emitter_index;
	...
	tfx_shared_properties_t &shared_properties = *ribbon_work_entry->shared_properties;
	...
	ribbon_work_entry->new_ribbons = tfx__new_ribbons_needed(pm, &ribbon_work_entry->random, emitter_index, &parent_effect, &shared_properties);
	...
	if (shared_properties.emission_type == tfxPath && ribbon_emitter.ribbon_property_flags & tfxRibbonPropertyFlags_static) {
		tfx__spawn_static_ribbons(&pm->work_queue, data);
	}

	ribbon_emitter.age += pm->frame_length;

	if (shared_properties.loop_length && ribbon_emitter.age > shared_properties.loop_length) {
		ribbon_emitter.age -= shared_properties.loop_length;
	}
...
void tfx__do_ribbon_spawn_work_3d(tfx_work_queue_t *queue, void *data) {
	tfx_ribbon_work_entry_t *work_entry = static_cast<tfx_ribbon_work_entry_t *>(data);
	tfx_particle_manager pm = work_entry->pm;
	tfx_ribbon_emitter_state_t &ribbon_emitter = pm->ribbon_emitters[work_entry->ribbon_emitter_index];
	tfx_shared_properties_t &shared_properties = *work_entry->shared_properties;
	if (shared_properties.emission_type == tfxPath && ribbon_emitter.ribbon_property_flags & tfxRibbonPropertyFlags_static) {
		tfx__spawn_ribbon_path_3d(queue, data);
	}
}

void tfx__do_ribbon_spawn_work_2d(tfx_work_queue_t *queue, void *data) {
	tfx_ribbon_work_entry_t *work_entry = static_cast<tfx_ribbon_work_entry_t *>(data);
	tfx_particle_manager pm = work_entry->pm;
	tfx_ribbon_emitter_state_t &ribbon_emitter = pm->ribbon_emitters[work_entry->ribbon_emitter_index];
	tfx_shared_properties_t &shared_properties = *work_entry->shared_properties;
...
void tfx__spawn_static_ribbons(tfx_work_queue_t *queue, void *data) {
	...
	if (ribbon_emitter.static_segment_start_index == tfxINVALID) {
		tfxU32 index = tfxRibbonBucketIndex(ribbon_emitter.segment_count);
		tfx_ribbon_bucket_t &ribbon_bucket = pm.ribbon_segment_buckets[index];
		ribbon_emitter.static_segment_start_index = ribbon_bucket.ribbons_buffer.current_size * ribbon_bucket.globals.segment_count;
		...
		if (total_vertex_count < pm.info.max_ribbon_segments * ribbon_bucket.globals.segment_count) {
			...
			pm.cached_static_path_segments.Insert(ribbon_emitter.path_hash + ribbon_emitter.segment_count, ribbon_emitter.static_segment_start_index);
		} else {
	...
	if (ribbon_emitter.static_segment_start_index != tfxINVALID) {
		tfxU32 bucket_index = tfxRibbonBucketIndex(ribbon_emitter.segment_count);
		tfx_ribbon_bucket_t *ribbon_bucket = &pm.ribbon_segment_buckets[bucket_index];
		tfxU32 actual_new_ribbons = 0;
		for (int i = 0; i != entry->amount_to_spawn; ++i) {
			tfxU32 ribbon_index = tfx__grab_ribbon(&pm, &ribbon_emitter);
...
void tfx__spawn_ribbon_path_3d(tfx_work_queue_t *queue, void *data) {
	...
	const tfx_ribbon_emitter_properties_t &properties = *entry->properties;
	tfx_emitter_path_t *path = &library->paths[ribbon_emitter.path_attributes];
	tfx_image_data_t *image = entry->shared_properties->image;
...
void tfx__init_common_particle_manager(tfx_particle_manager pm, tfxU32 max_particles, unsigned int effects_limit, tfx_particle_manager_mode mode, bool double_buffered_sprites, bool dynamic_sprite_allocation, bool group_sprites_by_effect, tfxU32 mt_batch_size) {
	...
	pm->control_ribbon_queue.reserve(pm->max_effects);
	...
	memset(&pm->ribbon_bucket_limits, 0, sizeof(tfxU32) * tfxArraySize_segment_bucket);
}
