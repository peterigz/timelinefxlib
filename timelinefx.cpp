	...
	for (auto &e : tfx_GetEffectInfo(effect)->sub_effectors) {
		if (e.property_flags & tfxEmitterPropertyFlags_enabled) {
			...
			if (e.type == tfxEmitterType) {
				...
				//Not enough buffer memory for the necessary segments.
				//Todo: Need to decide how to handle feedback to the user for this.
				TFX_ASSERT(pm->running_segment_count + ribbon_properties->segment_count <= pm->info.max_ribbon_segments);
				...
				if (ribbon_properties->emission_type == tfxPath) {
					...
					path_state.path_cycle_count = path->maximum_paths;
					path_state.path_start_index = 0;
					if (ribbon_emitter.state_flags & tfxEmitterStateFlags_has_rotated_path) {
						for (int qi = 0; qi != path->maximum_active_paths; ++qi) {
							path_state.path_quaternions[qi].cycles = tfxINVALID;
						}
						for (int qi = 0; qi != path_state.active_paths; ++qi) {
							...
							path_state.path_quaternions[qi].grid_coord = (ribbon_emitter.property_flags & tfxEmitterPropertyFlags_grid_spawn_clockwise) ? 0.f : (float)path->node_count - 4;
							path_state.path_quaternions[qi].age = 0.f;
							path_state.path_quaternions[qi].cycles = 0;
							if (path_state.path_cycle_count > 0) {
								path_state.path_cycle_count--;
...
void tfx__control_ribbon_path_age(tfx_work_queue_t *queue, void *data) {
	...
	if (bucket.highest_ribbon_index >= bucket.lowest_ribbon_index) {
		...
		pm.ribbon_buffer_requirements.segment_buffer_size_in_bytes += ribbon_count * ribbon_emitter.segment_count * sizeof(tfx_ribbon_segment_t);
...
void tfx_CopyRibbonDataToStagingBuffers(tfx_particle_manager pm, void *segments_dst, void *ribbons_dst) {
	...
	for (tfx_ribbon_bucket_t &bucket : pm->ribbon_segment_buckets) {
		if (bucket.active_ribbons) {
			tfxU32 ribbon_count = bucket.highest_ribbon_index - bucket.lowest_ribbon_index + 1;
			tfxU32 segment_range_size = bucket.globals.segment_count * sizeof(tfx_ribbon_segment_t);
			memcpy((tfx_ribbon_segment_t*)segments_dst + running_segment_offset, bucket.segments.data + (bucket.lowest_ribbon_index * bucket.globals.segment_count), ribbon_count * segment_range_size);
...
tfxU32 tfx__grab_ribbon(tfx_particle_manager pm, tfxU32 segment_count) {
	tfxU32 index = tfxRibbonBucketIndex(segment_count);
	...
	tfxU32 start_index = bucket.ribbons_buffer.current_size * bucket.globals.segment_count;
	pm->running_segment_count += bucket.globals.segment_count;
	if (pm->running_segment_count < pm->info.max_ribbon_segments) {
		...
		bucket.active_ribbons++;
		bucket.segments.resize((index + 1) *bucket.globals.segment_count);
		return index;
	} else {
		int d = 0;
	}
...
void tfx__update_ribbon_emitter(tfx_work_queue_t *work_queue, void *data) {
	...
	if (properties.emission_type == tfxPath && ribbon_emitter.property_flags & tfxRibbonPropertyFlags_static) {
		tfx__spawn_ribbons(&pm->work_queue, data);
	}
}
...
void tfx__spawn_ribbons(tfx_work_queue_t *queue, void *data) {
	...
	if (ribbon_emitter.path_state.active_paths != ribbon_emitter.active_ribbons) {
		tfxU32 bucket_index = tfxRibbonBucketIndex(ribbon_emitter.segment_count);
		tfx_ribbon_bucket_t *ribbon_bucket = &pm.ribbon_segment_buckets[bucket_index];
		entry->new_ribbons = ribbon_emitter.path_state.active_paths - ribbon_emitter.active_ribbons;
		tfxU32 actual_new_ribbons = 0;
		for (int i = 0; i != entry->new_ribbons; ++i) {
			tfxU32 ribbon_index = tfx__grab_ribbon(&pm, ribbon_emitter.segment_count);
...
void tfx__spawn_ribbon_path_3d(tfx_work_queue_t *queue, void *data) {
	...
	if (entry->new_ribbons > 0) {

		for (tfxU32 i = ribbon_emitter.ribbon_indexes[pm.current_ebuff].current_size - entry->new_ribbons; i != ribbon_emitter.ribbon_indexes[pm.current_ebuff].current_size; ++i) {
			tfxU32 ribbon_index = ribbon_emitter.ribbon_indexes[pm.current_ebuff][i];
			...
			if (ribbon_emitter.property_flags & tfxRibbonPropertyFlags_static) {
				...
				tfxWideFloat world_x = tfxWideSetSingle(ribbon_emitter.world_position.x + float(i) * 1.f);
				...
				for (tfxU32 s = 0; s < ribbon_emitter.segment_count; s += tfxDataWidth) {
					...
					tfxU32 segment_offset = ribbon_index * ribbon_emitter.segment_count;
					for (int j = 0; j != tfxDataWidth; ++j) {
						tfxU32 segment_index = s + j + segment_offset;
				...
				for (tfxU32 s = 0; s < ribbon_emitter.segment_count * ribbon_emitter.path_state.active_paths; ++s) {

					tfxU32 path_index = s / ribbon_emitter.segment_count;
					TFX_ASSERT(path_index < path->maximum_active_paths);

					segments[s].position_and_width = path[path_index].nodes[0];
					segments[s].position_and_width.w = 0.05f;
				}
			}
		}
	}
