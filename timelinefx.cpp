...
TFX_ENABLE_COMPILER_WARNING()
	...
	tfx__update_sprite_alignment_data(sprite_data, (float)pm->update_time);
...
tfxEffectID tfx__add_effect_to_effect_manager(tfx_effect_manager pm, tfx_effect_descriptor effect, int buffer, tfxU32 root_effect_index, float add_delayed_spawning) {
	...
	pm->effects[parent_index.index].highest_particle_age = (float)pm->frame_length * 3.f;
		...
		if (child->state_properties.shared_flags & tfxSharedEmitterPropertyFlags_enabled && !(child->state_properties.shared_flags & tfxSharedEmitterPropertyFlags_hidden)) {
			...
			if (child->type == tfxEmitterType) {
				...
				*/
				...
				emitter.highest_particle_age = (float)pm->frame_length * 2.f;
...
void tfx__update_library_control_profiles(tfx_library library) {
	...
	while (stack.size()) {
		...
		if (current->type == tfxEmitterType) {
			...
			tfx__update_emitter_state_flags(current);
		} else if (current->type == tfxRibbonType) {
			tfx__update_emitter_state_flags(current);
		}
		for (tfx_effect_descriptor sub : current->children) {
			stack.push_back(sub);
...
void tfx__update_effect_manager(void *data) {
	...
	for (int i = 0; i != effects_start_size; ++i) {
		...
		for (int emitter_index : pm->effects[effect_index.index].emitter_indexes[pm->current_ebuff]) {
			...
			spawn_work_entry->properties = &library->emitter_properties[pm->emitters[emitter_index].state_properties.property_index];
			spawn_work_entry->shared_properties = &library->shared_properties[pm->emitters[emitter_index].state_properties.shared_index];
			...
			if (timeout_counter <= pm->emitters[emitter_index].timeout) {
				...
				tfx__sync_lock(&pm->add_effect_mutex);
				if (pm->emitters[emitter_index].path_state.path_quaternions) {
					tfx__free_path_quaternion(pm, pm->emitters[emitter_index].path_state.path_quaternion_index);
				}
				...
				{
					tfx_particle_emitter_state_t &dying_emitter = pm->emitters[emitter_index];
					if (dying_emitter.state_properties.gpu_group_index != tfxINVALID) {
						pm->gpu_groups[dying_emitter.state_properties.gpu_group_index].active_emitter_count--;
						dying_emitter.state_properties.gpu_group_index = tfxINVALID;
					}
				}
	...
	for (auto &work_entry : pm->spawn_work) {
		...
		pm->effects[pm->emitters[index].parent_index].highest_particle_age = pm->emitters[index].highest_particle_age + (float)pm->frame_length;
	}
	pm->spawn_work.clear();
...
void tfx__control_ribbon_attributes(tfx_work_queue_t *queue, void *data) {
	...
	tfx_common_state_properties_t &state_properties = ribbon_emitter.state_properties;
	...
	for (tfxU32 ribbon_index : ribbon_emitter.ribbon_indexes[pm.current_ebuff]) {
		...
		image_frame += ribbon_emitter.state_properties.image_frame_rate * (float)pm.update_time;
...
void tfx_CopyRibbonDataToStagingBuffers(tfx_effect_manager effect_manager, void *segments_dst, void *ribbons_dst, void *emitters_dst) {
	//Make sure that you setup the ribbon buffers
	TFX_ASSERT_HANDLE(effect_manager); //If effect manager count is greater then 0 then effect_managers must be an array of tfx_effect_manager
	...
	if (segments_dst && ribbons_dst) {
		...
		tfx__wait_for_effect_manager_update(effect_manager);
		tfx_ribbon_bucket_t *bucket = effect_manager->ribbon_segment_buckets.next_item();
		while (bucket) {
			if (bucket->active_ribbons == 0) {
...
tfxU32 tfx__get_emitter_slot(tfx_effect_manager pm) {
...
void tfx__update_effect(tfx_effect_manager pm, tfxU32 index, tfxU32 parent_index) {
	...
	effect.highest_particle_age -= (float)pm->frame_length;
	...
	if (effect.highest_particle_age <= 0 && effect.age > (float)pm->frame_length * 5.f) {
...
void tfx__update_ribbon_emitter(tfxU32 ribbon_emitter_index, tfx_work_queue_t *work_queue, void *data) {
	...
	double step_size = 1.0 / ribbon_emitter.spawn_quantity;
	double tween = 0;
	...
	if (tween >= 1) {
		ribbon_emitter.amount_remainder = tween - 1.0;
	} else if (!(ribbon_emitter.state_properties.shared_flags & tfxSharedEmitterPropertyFlags_single)) {
		double amount_that_will_spawn = (1.0 - tween) / step_size;
		ribbon_work_entry->amount_to_spawn = (tfxU32)ceil(amount_that_will_spawn);
		if (ribbon_work_entry->amount_to_spawn > ribbon_work_entry->new_ribbons) {
			...
			ribbon_emitter.amount_remainder = 0.0;
		} else {
			ribbon_emitter.amount_remainder = amount_that_will_spawn - (tfxU32)amount_that_will_spawn;
			ribbon_emitter.amount_remainder = (1.0 - ribbon_emitter.amount_remainder) * step_size;
		}
	} else {
	...
	ribbon_emitter.age += (float)pm->frame_length;
	...
	if (ribbon_emitter.ribbon_indexes[pm->current_ebuff].current_size == 0 && ribbon_emitter.age > (float)pm->frame_length * 5.f) {
		ribbon_emitter.timeout_counter += (float)pm->frame_length;
	} else {
...
void tfx__update_emitter(tfx_work_queue_t *work_queue, void *data) {
	...
	if ((emitter.state_flags & tfxEmitterStateFlags_is_line_loop_or_kill && emitter.state_flags & tfxEmitterStateFlags_loop) || (emitter.state_flags & tfxEmitterStateFlags_is_single && shared_properties.single_shot_limit > 1)) {
		pm->emitters_check_capture.push_back(emitter_index);
	}

	//bool is_compute = emitter.state_properties.property_flags & tfxEmitterPropertyFlags_is_bottom_emitter && pm->flags & tfxEffectManagerFlags_use_compute_shader;
	...
	if (!(emitter.state_properties.shared_flags & tfxSharedEmitterPropertyFlags_single) || (emitter.state_properties.shared_flags & tfxSharedEmitterPropertyFlags_single && shared_properties.single_shot_limit > 0) || emitter.state_flags & tfxEmitterStateFlags_stop_spawning) {
		emitter.highest_particle_age -= (float)pm->frame_length;
		spawn_work_entry->highest_particle_age -= (float)pm->frame_length;
	}
	...
	if (emitter.highest_particle_age <= 0 && emitter.age > (float)pm->frame_length * 5.f) {
		emitter.timeout_counter += (float)pm->frame_length;
	}
	else {
		emitter.timeout_counter = 0;
	}
...
tfxU32 tfx__new_sprites_needed(tfx_effect_manager pm, tfx_random_t *random, tfxU32 index, tfx_effect_state_t *parent, tfx_shared_properties_t *shared_properties) {
	tfx_particle_emitter_state_t &emitter = pm->emitters[index];
	tfx_library library = emitter.library;
	...
	if (emitter.age == 0.f && emitter.spawn_quantity < 1.0) {
		emitter.amount_remainder = 1.0 + emitter.spawn_quantity;
		emitter.qty_step_size = step_size;
	}

	return emitter.amount_remainder >= 1.0 ? 0 : tfxU32((1.0 - emitter.amount_remainder) / step_size) + 1;
}
...
tfxU32 tfx__new_ribbons_needed(tfx_effect_manager pm, tfx_random_t *random, tfxU32 index, tfx_effect_state_t *parent, tfx_shared_properties_t *shared_properties) {
	...
	if (!(ribbon_emitter.state_properties.shared_flags & tfxSharedEmitterPropertyFlags_single)) {
		...
		step_size = 1.0 / ribbon_emitter.spawn_quantity;
	} else {
		step_size = 1.0 / ribbon_emitter.spawn_quantity;
	}

	if (ribbon_emitter.age == 0.f && ribbon_emitter.spawn_quantity < 1.0) {
		ribbon_emitter.amount_remainder = 1.0 + ribbon_emitter.spawn_quantity;
		ribbon_emitter.qty_step_size = step_size;
	}
	return ribbon_emitter.amount_remainder >= 1.0 ? 0 : tfxU32((1.0 - ribbon_emitter.amount_remainder) / step_size) + 1;
}
...
void tfx__spawn_particles(tfx_effect_manager pm, tfx_spawn_work_entry_t *work_entry) {
	...
	double step_size = 1.0 / emitter.spawn_quantity;
	double tween = 0;
	if (step_size == emitter.qty_step_size || emitter.state_properties.shared_flags & tfxSharedEmitterPropertyFlags_single) {
	...
	work_entry->spawn_start_index = tfx__add_soa_rows_grew(&pm->particle_array_buffers[emitter.particles_index], work_entry->amount_to_spawn, true, grew);
...
void tfx__do_spawn_work(tfx_work_queue_t *queue, void *data) {
	...
	if (work_entry->emission_type == tfxOtherEmitter) {
		if (emitter.state_properties.shared_flags & tfxSharedEmitterPropertyFlags_single) {
			...
			tfx__spawn_particle_other_emitter(&pm->work_queue, work_entry);
		}
	} else if (work_entry->emission_type == tfxSpawnOnRibbon) {
		...
		tfx__spawn_particle_line_start(&pm->work_queue, work_entry);
	} else if (work_entry->emission_type == tfxIcosphere && emitter.state_properties.shared_flags & tfxSharedEmitterPropertyFlags_grid_spawn_random) {
	...
	tfx__spawn_particle_micro_update(&pm->work_queue, work_entry);
	if (emitter.state_properties.control_profile & tfxEmitterControlProfile_has_simplex_noise_type) {
...
void tfx__spawn_particle_age(tfx_work_queue_t *queue, void *data) {
	...
	float frame_fraction = (float)entry->pm->frame_length / (float)entry->amount_to_spawn;
	...
	for (int i = 0; i != entry->amount_to_spawn; ++i) {
		...
		if ((entry->emission_type == tfxOtherEmitter && emitter.other_emitter_index == tfxINVALID) || (entry->emission_type == tfxSpawnOnRibbon)) {
			...
			max_age *= factor_life;
			max_age = tfx__Max(max_age, (float)pm.frame_length);
		...
		entry->highest_particle_age = fmaxf(entry->highest_particle_age, (max_age * loop_count) + (float)pm.frame_length + 1);
	}
}
...
void tfx__spawn_particle_size(tfx_work_queue_t *queue, void *data) {
	tfxPROFILE;
...
void tfx__spawn_particle_spin(tfx_work_queue_t *queue, void *data) {
	tfxPROFILE;
...
void tfx__spawn_particle_point(tfx_work_queue_t *queue, void *data) {
	...
	for (int i = 0; i != entry->amount_to_spawn; ++i) {
		...
		tween = 1.f - entry->particle_data->age[index] / (float)pm.frame_length;
...
void tfx__spawn_particle_other_emitter(tfx_work_queue_t *queue, void *data) {
	tfxPROFILE;
	tfx_spawn_work_entry_t *entry = static_cast<tfx_spawn_work_entry_t *>(data);
	tfx_random_t random = entry->random;
	double tween = 0.0;
	...
	for (int i = 0; i != entry->amount_to_spawn; ++i) {
		...
		if (!(emitter.state_properties.shared_flags & tfxSharedEmitterPropertyFlags_relative_position)) {
			tween = 1.f - entry->particle_data->age[index] / (float)pm.frame_length;
			float lerp_position_x = tfx__interpolate_float((float)tween, spawn_points.captured_position_x[spawn_index], spawn_points.position_x[spawn_index]);
			float lerp_position_y = tfx__interpolate_float((float)tween, spawn_points.captured_position_y[spawn_index], spawn_points.position_y[spawn_index]);
			float lerp_position_z = tfx__interpolate_float((float)tween, spawn_points.captured_position_z[spawn_index], spawn_points.position_z[spawn_index]);
			if (emitter.state_properties.shared_flags & tfxSharedEmitterPropertyFlags_emitter_handle_auto_center) {
...
void tfx__spawn_particle_other_emitter_single(tfx_work_queue_t *queue, void *data) {
	tfxPROFILE;
	tfx_spawn_work_entry_t *entry = static_cast<tfx_spawn_work_entry_t *>(data);
	tfx_random_t random = entry->random;
	double tween = 0.0;
...
void tfx__spawn_particle_line(tfx_work_queue_t *queue, void *data) {
	tfxPROFILE;
	tfx_spawn_work_entry_t *entry = static_cast<tfx_spawn_work_entry_t *>(data);
	tfx_random_t random = entry->random;
	double tween = entry->tween;
...
void tfx__spawn_particle_line_start(tfx_work_queue_t *queue, void *data) {
	tfxPROFILE;
	tfx_spawn_work_entry_t *entry = static_cast<tfx_spawn_work_entry_t *>(data);
	tfx_random_t random = entry->random;
	double tween = entry->tween;
...
void tfx__spawn_particle_area(tfx_work_queue_t *queue, void *data) {
	tfxPROFILE;
	tfx_spawn_work_entry_t *entry = static_cast<tfx_spawn_work_entry_t *>(data);
	tfx_random_t random = entry->random;
	double tween = entry->tween;
...
void tfx__spawn_particle_path_start(tfx_work_queue_t *queue, void *data) {
	tfxPROFILE;
	tfx_spawn_work_entry_t *entry = static_cast<tfx_spawn_work_entry_t *>(data);
	tfx_random_t random = entry->random;
	double tween = entry->tween;
...
void tfx__spawn_particle_path(tfx_work_queue_t *queue, void *data) {
	tfxPROFILE;
	tfx_spawn_work_entry_t *entry = static_cast<tfx_spawn_work_entry_t *>(data);
	tfx_random_t random = entry->random;
	double tween = entry->tween;
...
void tfx__spawn_static_ribbons(tfxU32 ribbon_emitter_index, tfx_work_queue_t *queue, void *data) {
	...
	if (ribbon_emitter.static_segment_start_index != tfxINVALID) {
		...
		for (int i = 0; i != entry->amount_to_spawn; ++i) {
			...
			actual_new_ribbons++;
			TFX_ASSERT(ribbon_index < ribbon_bucket->ribbons_buffer.current_size);
...
void tfx__spawn_particle_micro_update(tfx_work_queue_t *queue, void *data) {
	tfxPROFILE;
	tfx_spawn_work_entry_t *entry = static_cast<tfx_spawn_work_entry_t *>(data);
	tfx_random_t random = entry->random;
	tfx_effect_manager_t &pm = *entry->pm;
