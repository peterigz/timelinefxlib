...
void RecordSpriteData(tfx_particle_manager_t *pm, tfx_effect_emitter_t *effect, float update_frequency, float camera_position[3], std::atomic_int *progress) {
	...
	for (tfxEachLayer) {
		pm->instance_buffer_for_recording[0][layer] = tfxCreateBuffer(struct_size);
		pm->instance_buffer_for_recording[1][layer] = tfxCreateBuffer(struct_size);
	}
	...
	while (frame < frames && offset < 99999) {
		...
		if (offset >= start_frame) {
			...
			for(tfxEachLayer) {
				...
				if (is_3d) {
					memcpy(sprite_data->real_time_sprites.billboard_instance + frame_meta[frame].index_offset[layer], billboards, sizeof(tfx_billboard_instance_t) * pm->layer_sizes[layer]);
				}
				else {
					memcpy(sprite_data->real_time_sprites.sprite_instance + frame_meta[frame].index_offset[layer], sprites, sizeof(tfx_sprite_instance_t) * pm->layer_sizes[layer]);
				}
...
void UpdateParticleManager(tfx_particle_manager_t *pm, float elapsed_time) {
	...
	*/
	...
	bool is_recording = (pm->flags & tfxParticleManagerFlags_recording_sprites) > 0;
	for (int depth = 0; depth != tfxMAXDEPTH; ++depth) {
		for (int index : pm->emitters_in_use[depth][next_buffer]) {
			...
			while (particles_to_update > 0) {
				...
				work_entry.sprite_instances = !is_recording ? &pm->instance_buffer[pm->current_sprite_buffer] : &pm->instance_buffer_for_recording[pm->current_sprite_buffer][work_entry.properties->layer];
			...
			for (tfxEachLayer) {
				for (auto &depth : sprites.depth_indexes[layer][sprites.current_depth_buffer_index[layer]]) {
					if (depth.particle_id != tfxINVALID) {
						...
						pm->particle_arrays[bank].depth_index[index] = sprites.depth_indexes[layer][!sprites.current_depth_buffer_index[layer]].current_size;
						sprites.depth_indexes[layer][!sprites.current_depth_buffer_index[layer]].push_back(depth);
					}
				}
...
void UpdatePMEmitter(tfx_work_queue_t *work_queue, void *data) {
	...
	bool is_recording = (pm->flags & tfxParticleManagerFlags_recording_sprites) > 0;
	tfx_buffer_t &instance_buffer = !is_recording ? pm->instance_buffer[pm->current_sprite_buffer] : pm->instance_buffer_for_recording[pm->current_sprite_buffer][layer];
	tfxU32 &sprite_index_point = pm->sprite_index_point[layer];
	tfx_vector_t<tfx_depth_index_t> &depth_indexes =  effect_sprites.depth_indexes[layer][effect_sprites.current_depth_buffer_index[layer]];
...
tfxAPI tfx_sprite_instance_t *GetEffectSpriteBuffer(tfx_particle_manager_t *pm, tfxEffectID effect_index, tfxU32 layer, tfxU32 *sprite_count) {
	...
	*sprite_count = pm->effects[effect_index].sprites.sprite_count;
	return tfxCastBufferRef(tfx_sprite_instance_t, pm->instance_buffer[pm->current_sprite_buffer]);
}
...
bool GetNextSpriteBuffer(tfx_particle_manager_t *pm, tfx_sprite_instance_t **sprite_instances, tfx_effect_sprites_t **effect_sprites, tfxU32 *sprite_count) {
	...
	*sprite_instances = tfxCastBufferRef(tfx_sprite_instance_t, pm->instance_buffer[pm->current_sprite_buffer]);
