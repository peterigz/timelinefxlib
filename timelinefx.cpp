...
tfx_effect_descriptor_t *tfx__add_emitter_to_effect(tfx_effect_descriptor_t *effect, tfx_effect_descriptor_t *emitter) {
...
void tfx__reset_emitter_graphs(tfx_effect_descriptor_t *effect, bool add_node, bool compile) {
	...
	tfx__reset_graph(&library->graphs[graph_list_index].graphs[tfxEmitter_overtime_blendfactor_index], 1.f, tfxOpacityOvertimePreset, add_node); library->graphs[graph_list_index].graphs[tfxEmitter_overtime_blendfactor_index].type = tfxOvertime_blendfactor;
...
void tfx__reset_ribbon_graphs(tfx_effect_descriptor_t *effect, bool add_node, bool compile) {
	...
	tfx__reset_graph(&library->graphs[graph_list_index].graphs[tfxRibbon_overtime_blendfactor_index], 1.f, tfxOpacityOvertimePreset, add_node); library->graphs[graph_list_index].graphs[tfxRibbon_overtime_blendfactor_index].type = tfxOvertime_blendfactor;
...
void tfx__initialise_unitialised_graphs(tfx_effect_descriptor_t *effect) {
	...
	if (effect->type == tfxEmitterType) {
		...
		if (library->graphs[graph_list_index].graphs[tfxEmitter_overtime_blendfactor_index].nodes.size() == 0) tfx__reset_graph(&library->graphs[graph_list_index].graphs[tfxEmitter_overtime_blendfactor_index], 1.f, tfxOpacityOvertimePreset);
	...
	if (effect->type == tfxRibbonType) {
		...
		if (library->graphs[graph_list_index].graphs[tfxRibbon_overtime_blendfactor_index].nodes.size() == 0) tfx__reset_graph(&library->graphs[graph_list_index].graphs[tfxRibbon_overtime_blendfactor_index], 1.f, tfxOpacityOvertimePreset);
...
tfxEffectID tfx__add_effect_to_particle_manager(tfx_particle_manager pm, tfx_effect_descriptor_t *effect, int buffer, int hierarchy_depth, bool is_sub_emitter, tfxU32 root_effect_index, float add_delayed_spawning) {
	...
	new_effect.ribbon_indexes[0].clear();
	new_effect.ribbon_indexes[1].clear();
...
void tfx_UpdateParticleManager(tfx_particle_manager pm, float elapsed_time) {
	...
	for (int depth = 0; depth != tfxMAXDEPTH; ++depth) {
		...
		for (int i = 0; i != effects_start_size[depth]; ++i) {
			...
			effect.ribbon_indexes[next_buffer].clear();
			...
			for (int ribbon_index : pm->effects[effect_index.index].ribbon_indexes[pm->current_ebuff]) {
...
void tfx__control_ribbons(tfx_work_queue_t *queue, void *data) {
	...
	tfx__control_ribbon_width(queue, data);
}

void tfx__control_ribbon_width(tfx_work_queue_t *queue, void *data) {
	tfxPROFILE;
	tfx_control_ribbon_work_entry_t *work_entry = static_cast<tfx_control_ribbon_work_entry_t *>(data);
	tfxU32 ribbon_index = work_entry->ribbon_index;
	tfx_particle_manager_t &pm = *work_entry->pm;
...
tfx_effect_index_t tfx__get_effect_slot(tfx_particle_manager pm) {
	...
	effect.ribbon_indexes[0].init();
	effect.ribbon_indexes[1].init();
...
void tfx__spawn_static_ribbons(tfx_work_queue_t *queue, void *data) {
	...
	const float base_width = tfx__lookup_precise(&graph_list.graphs[tfxRibbon_base_width_index], ribbon_emitter.frame) * entry->parent_spawn_controls->size_x;
	...
	if (ribbon_emitter.static_segment_start_index == tfxINVALID) {
		...
		if (total_vertex_count < pm.info.max_ribbon_segments * ribbon_bucket.globals.segment_count) {
			...
			tfxU32 texture_indexes = (tfxColorRampIndex(entry->graphs->color_ramp_bitmap_indexes) << 24) | (tfxColorRampLayer(entry->graphs->color_ramp_bitmap_indexes) << 16) | (entry->shared_properties->image->compute_shape_index + (tfxU32)entry->shared_properties->start_frame);
